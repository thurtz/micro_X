...
OLLAMA_MODEL = 'vitali87/shell-commands-qwen2-1.5b' # Main model for translation
...
async def is_valid_linux_command_according_to_ai(command_text: str) -> bool | None:
    ...
    prompt = f"""Critically assess if the following string, *exactly as written*, is a complete, directly executable Linux command or an absolute/relative path to an executable.
Natural language phrases, questions, or incomplete commands are NOT valid.
If there is any doubt, answer 'no'.
Answer only with the single word 'yes' or the single word 'no'.
...
async def handle_input_async(user_input: str):
    ...
    if user_input_stripped.startswith("/ai "):
        ...
        # Get a command translated and validated by AI
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        if is_cmd_ai_says is True and not user_input_looks_like_phrase:
            ...
            # Attempt translation and validation
...
# Regex to extract commands wrapped in various tags or markdown code blocks
# Tries to capture content within <bash>, <code>, <pre>, <command>, <cmd>, ```bash, ```
# Also captures content within <unsafe> tag separately.
...
# e.g., if group 2 captured "<bash>ls -l</bash>" instead of just "ls -l"
...
async def _interpret_and_clean_ai_output(human_input: str) -> tuple[str | None, str | None]:
    """
    Calls the translation AI, parses the response, and cleans the extracted command.
    ...
    if COMMAND_PATTERN is None:
        logger.error("COMMAND_PATTERN regex is not compiled. Cannot interpret AI output.")
        return None, None

    raw_candidate_from_regex = None # Store the first matched group content
    ollama_call_retries = 2 # Number of times to retry the Ollama call itself on connection errors etc.
...
If the request is ambiguous, unsafe, or cannot be translated into a single command, respond with only "<unsafe>Cannot translate safely</unsafe>" or a similar message within <unsafe> tags."""
            ...
            if match:
                ...
                if COMMAND_PATTERN.groups >= _UNSAFE_TAG_CONTENT_GROUP and match.group(_UNSAFE_TAG_CONTENT_GROUP) is not None:
                    ...
                    logger.warning(f"Translation AI indicated unsafe query: '{human_input}'. AI Msg: '{unsafe_message}'")
                    append_output(f"⚠️ AI: {unsafe_message}")
                ...
                for group_index in _COMMAND_EXTRACT_GROUPS:
                    ...
                    if COMMAND_PATTERN.groups >= group_index and (extracted_candidate := match.group(group_index)) is not None:
                        ...
                        # Start processing the extracted candidate
                        ...
                        if inner_match:
                            ...
                            # Only strip if it's one of the expected tags
                            if tag_name in ["bash", "code", "cmd", "command", "pre"]:
                                extracted_content = inner_match.group(2).strip()
                                logger.debug(f"Stripped inner tag <{tag_name}>: '{processed_candidate}' -> '{extracted_content}'")
                                processed_candidate = extracted_content
                            else:
                                logger.debug(f"Inner tag <{tag_name}> found but not one of the expected types to strip. Original: '{processed_candidate}'")
                        ...
                        if len(processed_candidate) >= 2:
                            if processed_candidate.startswith("'") and processed_candidate.endswith("'"):
                                ...
                                logger.debug(f"Stripped outer quotes from '{extracted_candidate.strip()}': -> '{processed_candidate}'")
                            elif processed_candidate.startswith("`") and processed_candidate.endswith("`"):
                                processed_candidate = processed_candidate[1:-1].strip()
                                logger.debug(f"Stripped outer backticks from '{extracted_candidate.strip()}': -> '{processed_candidate}'")
                        ...
                        if (processed_candidate.lower().startswith("bash ") or processed_candidate.lower().startswith("sh ")) and len(processed_candidate) > 6:
                             ...
                             # Check if the part after "bash " looks like <...>
                             if potential_inner_cmd.startswith("<") and potential_inner_cmd.endswith(">") and len(potential_inner_cmd) >=2:
                                 ...
                                 # Only strip if the content inside <> doesn't have shell metacharacters
                                 if not any(c in inner_cmd_content for c in '<>|&;'):
                                     logger.debug(f"Stripped '{processed_candidate[:prefix_len]}<cmd>' pattern: '{processed_candidate}' -> '{inner_cmd_content}'")
                                     processed_candidate = inner_cmd_content
                                 else:
                                     logger.debug(f"Retained '{processed_candidate[:prefix_len]}<cmd>' structure due to special chars: '{processed_candidate}'")
                        ...
                        if len(processed_candidate) >= 2 and processed_candidate.startswith("<") and processed_candidate.endswith(">"):
                            ...
                            # Check again for shell metacharacters
                            if not any(c in inner_content for c in '<>|&;'):
                                logger.debug(f"Stripped general angle brackets: '{processed_candidate}' -> '{inner_content}'")
                                processed_candidate = inner_content
                            else:
                                logger.debug(f"Retained general angle brackets due to special chars: '{processed_candidate}'")


                        # 5. Strip leading slash if it's the only slash (e.g., "/ls" -> "ls")
                        # Avoid stripping if it's a path like "/bin/ls"
                        cleaned_linux_command = processed_candidate.strip()
                        if cleaned_linux_command.startswith('/') and '/' not in cleaned_linux_command[1:]:
                            original_for_log = cleaned_linux_command
                        ...
                        # This is a basic heuristic and might not cover all cases (e.g., complex pipes)
                        original_for_multicmd_log = cleaned_linux_command
                        try:
                            # Match characters up to the first command separator (;, &&, ||)
                            ...
                            if first_command_match:
                                ...
                                if first_command_part != cleaned_linux_command:
                                    # If we truncated the command, log it
                                ...
                                # Contains separators but couldn't extract first part cleanly - likely problematic
                                ...
                                cleaned_linux_command = "" # Discard the command
                             ...
                             cleaned_linux_command = original_for_multicmd_log # Use the command as extracted
                        ...
                        if cleaned_linux_command and not cleaned_linux_command.lower().startswith(("sorry", "i cannot", "unable to", "cannot translate")):
                            logger.debug(f"_interpret_and_clean_ai_output returning: Cleaned='{cleaned_linux_command}', Raw='{raw_candidate_from_regex}'")
                ...
                logger.warning(f"Translation AI response matched pattern but no valid command extracted after cleaning. Response: {ai_response}")

            else:
                # AI response didn't match any known patterns
                logger.error(f"Translation AI response did not match expected patterns. Response: {ai_response}")
            ...
            if attempt < ollama_call_retries:
                ...
                logger.error(f"Translation AI parsing/match failed for '{human_input}' after {ollama_call_retries+1} internal attempts. Last AI response: {ai_response}")
        ...
        # --- Exception Handling for Ollama Call ---
            ...
            # Error response from the Ollama API (e.g., model not found, server error)
            current_attempt_exception = e_resp
            append_output(f"❌ Ollama API Error (Translation): {e_resp}")
            logger.error(f"Ollama API Error (Translation): {e_resp}", exc_info=True)
            # Don't retry on API errors, assume it's persistent for this request
            ...
            append_output(f"❌ Ollama Connection Error (Translation): {e_req}")
            logger.error(f"Ollama Connection Error (Translation): {e_req}", exc_info=True)
            # Fall through to retry logic below
        except Exception as e_gen:
            # Other unexpected errors during the process
            current_attempt_exception = e_gen
            append_output(f"❌ AI Processing Error (Translation): {e_gen}")
            logger.exception(f"Unexpected error in _interpret_and_clean_ai_output for '{human_input}'")
            # Fall through to retry logic below
        ...
        if current_attempt_exception:
            ...
            # Retry if attempts remain and it wasn't a non-recoverable API error
            if attempt < ollama_call_retries and not isinstance(current_attempt_exception, ollama.ResponseError):
                ...
                await asyncio.sleep(retry_delay_seconds)
                # continue is implicit here, loop will proceed
            else:
                # All retries failed or it was a non-retriable error
                logger.error(f"All Translation AI call attempts failed for '{human_input}'. Last error: {current_attempt_exception}")
                return None, raw_candidate_from_regex # Return failure

    # Should not be reachable if loop logic is correct, but as a safeguard:
...
async def get_validated_ai_command(human_query: str) -> tuple[str | None, str | None]:
    """
    Attempts to get a Linux command from AI translation and validate it using the Validator AI.
    Retries the entire translation-validation cycle multiple times.
    ...
    Returns:
        ...
        - last_raw_candidate: The last raw candidate extracted by the translation regex, or None.
    """
    logger.info(f"Attempting validated translation for: '{human_query}'")
    last_raw_candidate = None # Store the last raw output from translation AI
    last_cleaned_command_attempt = None # Store the last successfully cleaned command

    # Loop through the translation-validation cycle
    for i in range(TRANSLATION_VALIDATION_CYCLES):
        ...
        # Step 1: Get cleaned command and raw candidate from the Translation AI
        cleaned_command, raw_candidate = await _interpret_and_clean_ai_output(human_query)

        # Store the latest raw candidate and cleaned command attempt
        if raw_candidate and last_raw_candidate is None: # Store the first non-None raw candidate
            last_raw_candidate = raw_candidate
        if cleaned_command:
            last_cleaned_command_attempt = cleaned_command # Store the latest successfully cleaned command

        # Step 2: If translation produced a command, validate it
        if cleaned_command:
            append_output(f"🤖 AI translated to: '{cleaned_command}'. Validating with AI Validator...")
            if get_app().is_running : get_app().invalidate()

            # Ask the Validator AI if the cleaned command is valid
            is_valid_by_validator = await is_valid_linux_command_according_to_ai(cleaned_command)

            if is_valid_by_validator is True:
                # Success! Validator confirmed the command.
                ...
                return cleaned_command, raw_candidate # Return the validated command
            ...
            # Translation step failed to produce a command candidate
            logger.warning(f"Main AI translation (cycle {i+1}) failed to produce a command for '{human_query}'.")

        # Step 3: If not successful yet and cycles remain, retry
        if i < TRANSLATION_VALIDATION_CYCLES - 1:
            ...
            # Return the *last cleaned command attempt* even if validation failed/was inconclusive,
            # along with the last raw candidate. The caller might decide to use it as a fallback.
