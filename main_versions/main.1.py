...
async def is_valid_linux_command_according_to_ai(command_text: str) -> bool | None:
    ...
    try:
        ...
        # Use regex to find "yes" or "no" as whole words, case-insensitive
        is_yes = re.search(r'\byes\b', ai_answer, re.IGNORECASE)
        is_no = re.search(r'\bno\b', ai_answer, re.IGNORECASE)

        if is_yes and not is_no: # Clearly "yes"
            return True
        elif is_no and not is_yes: # Clearly "no"
            return False
        else: # Ambiguous (e.g., contains both, or neither, or other text)
            logger.warning(f"Validator AI gave unclear or ambiguous answer for '{command_text}': '{ai_answer}'")
            return None 
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        if is_cmd_ai_says is True: 
            ...
            if is_cmd_ai_says is False:
                ...
                append_output(f"⚠️ AI validation for '{user_input}' was inconclusive. Attempting as natural language query...")
                logger.warning(f"Validator AI response for '{user_input}' inconclusive. Defaulting to natural language query.")
...
async def process_command(command_str_original: str, original_user_input: str, ai_raw_candidate: str | None = None):
    ...
    command_to_execute_sanitized = sanitize_and_validate(command_to_execute_expanded, original_user_input)
    ...
    if category == "simple":
        execute_shell_command(command_to_execute_sanitized, original_user_input)
    else: 
