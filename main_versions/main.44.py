...
# REMOVED: categorization_flow_active = False
# REMOVED: categorization_flow_state = {}
confirmation_flow_active = False # This remains as it's for a separate flow
confirmation_flow_state = {}   # This remains
...
def _handle_exit_or_cancel(event):
    # global categorization_flow_active, categorization_flow_state, confirmation_flow_active, confirmation_flow_state # Removed categorization globals
    global confirmation_flow_active, confirmation_flow_state # Keep confirmation globals
    global ui_manager_instance

    # Access categorization state through ui_manager_instance
    is_cat_active = ui_manager_instance.categorization_flow_active if ui_manager_instance else False
    is_conf_active = ui_manager_instance.confirmation_flow_active if ui_manager_instance else confirmation_flow_active # Or keep as global for now
    ...
    if is_cat_active:
        ...
        # ui_manager_instance.categorization_flow_active = False # UIManager's finally block will handle this
    ...
    elif is_conf_active: # Confirmation flow logic remains largely unchanged for now
        if ui_manager_instance: ui_manager_instance.append_output("\n⚠️ Command confirmation cancelled by user.", style_class='warning')
        logger.info("Confirmation flow cancelled by Ctrl+C/D.")
        confirmation_flow_active = False # Or ui_manager_instance.confirmation_flow_active = False
...
def restore_normal_input_handler():
    ...
    # global categorization_flow_active, confirmation_flow_active # categorization_flow_active is now in UIManager
    global confirmation_flow_active # Keep confirmation_flow_active for now

    # categorization_flow_active = False # Now handled by UIManager
    confirmation_flow_active = False # Or ui_manager_instance.confirmation_flow_active

    if ui_manager_instance:
        # UIManager's set_normal_input_mode will handle its internal categorization_flow_active flag
...
async def handle_input_async(user_input: str):
    global ui_manager_instance, current_directory, ollama_service_ready, confirmation_flow_active # categorization_flow_active removed from globals
    if not ui_manager_instance: logger.error("handle_input_async: UIManager not initialized."); return
    append_output_func = ui_manager_instance.append_output
    # Access categorization_flow_active through ui_manager_instance
    is_cat_active = ui_manager_instance.categorization_flow_active
    is_conf_active = ui_manager_instance.confirmation_flow_active # Or confirmation_flow_active global
...
async def process_command(command_str_original: str, original_user_input_for_display: str,
    ...
    # global categorization_flow_active, categorization_flow_state # Removed categorization globals
    global ui_manager_instance, current_directory, confirmation_flow_active, confirmation_flow_state
    ...
    if not category:
        ...
        if category == UNKNOWN_CATEGORY_SENTINEL:
            ...
            # Call the updated prompt_for_categorization which now uses UIManager
...
async def prompt_for_command_confirmation(command_to_confirm: str, display_source: str) -> dict:
    global confirmation_flow_active, confirmation_flow_state, ui_manager_instance, config # Keep confirmation_flow_active & state for now
    if not ui_manager_instance: logger.error("prompt_for_command_confirmation: UIManager not initialized."); return {'action': 'cancel'}

    confirmation_flow_active = True # Or ui_manager_instance.confirmation_flow_active = True
    ...
    action_taken = None
    try:
        result = await confirmation_flow_state['future']
        action_taken = result.get('action')
        return result
    finally:
        confirmation_flow_active = False # Or ui_manager_instance.confirmation_flow_active = False
        if ui_manager_instance:
            ui_manager_instance.confirmation_flow_active = False # Ensure UIManager's flag is also reset if it owns it

        if action_taken == 'edit_mode_engaged':
            logger.info("Modify action chosen: Setting up edit mode via UIManager.")
            if ui_manager_instance:
                command_for_edit = confirmation_flow_state.get('command_to_confirm', '')
...
def _ask_confirmation_main_choice():
    global confirmation_flow_state, ui_manager_instance # Keep confirmation_flow_state for now
    ...
    ui_manager_instance.set_flow_input_mode(
        ...
        accept_handler_func=_handle_confirmation_main_choice_response,
        is_confirmation=True # This flag in UIManager can set its internal confirmation_flow_active
    )
...
def _handle_confirmation_main_choice_response(buff):
    ...
    if response in ['y', 'yes']: confirmation_flow_state['future'].set_result({'action': 'execute', 'command': confirmation_flow_state['command_to_confirm']}); valid_choice_made = True
        ...
        confirmation_flow_state['future'].set_result({'action': 'edit_mode_engaged'})
...
async def _handle_explain_command_async():
    global confirmation_flow_state, ui_manager_instance, config # Keep confirmation_flow_state for now
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    ...
    current_app_inst = ui_manager_instance.get_app_instance()
    if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
    explanation = await explain_linux_command_with_ai(command_to_explain, config, append_output_func)
...
def _ask_confirmation_after_explain():
    ...
    ui_manager_instance.set_flow_input_mode(
        ...
        accept_handler_func=_handle_confirmation_after_explain_response,
        is_confirmation=True # This flag in UIManager can set its internal confirmation_flow_active
    )
...
async def prompt_for_categorization(command_initially_proposed: str,
    ...
    """
    Initiates the command categorization flow via UIManager.
    The actual flow steps (_ask_step_..., _handle_step_...) are still in main.py for Phase 1,
    but they will use the state managed by UIManager.
    """
    global ui_manager_instance, config, current_directory # current_directory needed for restore_normal_input_handler
    ...
    # UIManager's start_categorization_flow will set up the state and future,
    # and for Phase 1, it will call _ask_step_0_5_confirm_command_base (from main.py)
    ...
    result = await ui_manager_instance.start_categorization_flow(
        ...
        _ask_step_0_5_confirm_command_base # Pass the first step function for UIManager to call
        # normal_input_accept_handler, # Not directly used by UIManager in Phase 1 for restoration
        # current_directory # Not directly used by UIManager in Phase 1 for restoration
    )
    logger.info(f"Main.py: Categorization flow result from UIManager: {result}")

    # The finally block for restoring normal input mode remains here in main.py for Phase 1.
    # It will be called after the UIManager's flow (and its internal future) completes.
    is_conf_active = ui_manager_instance.confirmation_flow_active if ui_manager_instance else confirmation_flow_active
    if not is_conf_active: # Only restore if not also in a confirmation flow (unlikely here, but good check)
...
# --- Helper functions for categorization flow (temporarily using ui_manager_instance state) ---
# These will be moved to UIManager in subsequent phases.

def _ask_step_0_5_confirm_command_base():
    # global categorization_flow_state, ui_manager_instance # No longer global state
    global ui_manager_instance # ui_manager_instance is global
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    # Access state through ui_manager_instance
    proposed = ui_manager_instance.categorization_flow_state['command_initially_proposed']
    original = ui_manager_instance.categorization_flow_state['original_direct_input']
    ...
    if original and original.strip() != proposed.strip():
        ...
        ui_manager_instance.set_flow_input_mode(
            ...
            accept_handler_func=_handle_step_0_5_response,
            is_categorization=True # This flag in UIManager can set its internal categorization_flow_active
        )
    else:
        ui_manager_instance.categorization_flow_state['command_to_add_final'] = proposed
        ui_manager_instance.categorization_flow_state['step'] = 1
        _ask_step_1_main_action() # This will also need to use ui_manager_instance.categorization_flow_state
...
def _handle_step_0_5_response(buff):
    ...
    if response == '1':
        ...
        ui_manager_instance.categorization_flow_state['command_to_add_final'] = original
        append_output_func(f"Using original: '{original}'", style_class='categorize-info')
        ui_manager_instance.categorization_flow_state['step'] = 1
        _ask_step_1_main_action()
    elif response == '3':
        ui_manager_instance.categorization_flow_state['step'] = 3.5
        _ask_step_3_5_enter_custom_command_for_categorization()
    elif response == '4':
        ui_manager_instance.categorization_flow_state.get('future').set_result({'action': 'cancel_execution'})
    else:
        append_output_func("Invalid choice (1-4). Please try again.", style_class='error')
        _ask_step_0_5_confirm_command_base() # Re-ask the current step
        return

def _ask_step_1_main_action():
    # global categorization_flow_state, ui_manager_instance, config # No longer global state
    global ui_manager_instance, config
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    # Access state through ui_manager_instance
    cmd_display = ui_manager_instance.categorization_flow_state['command_to_add_final']
    ...
    ui_manager_instance.set_flow_input_mode(
        ...
        accept_handler_func=_handle_step_1_main_action_response,
        is_categorization=True # This flag in UIManager can set its internal categorization_flow_active
    )
...
def _handle_step_1_main_action_response(buff):
    ...
    # Access state through ui_manager_instance
    cmd_to_add = ui_manager_instance.categorization_flow_state['command_to_add_final']
    future_to_set = ui_manager_instance.categorization_flow_state.get('future')

    if response in ['1', '2', '3']:
        chosen_category = CM_CATEGORY_MAP.get(response)
        if future_to_set: future_to_set.set_result({'action': 'categorize_and_execute', 'command': cmd_to_add, 'category': chosen_category})
    elif response == 'm':
        ui_manager_instance.categorization_flow_state['step'] = 4
        _ask_step_4_enter_modified_command(base_command=cmd_to_add)
    elif response == 'd':
        if future_to_set: future_to_set.set_result({'action': 'execute_as_default'})
    elif response == 'c':
        if future_to_set: future_to_set.set_result({'action': 'cancel_execution'})
    else:
...
def _ask_step_3_5_enter_custom_command_for_categorization():
    ...
    ui_manager_instance.set_flow_input_mode(
        ...
        accept_handler_func=_handle_step_3_5_response,
        is_categorization=True # This flag in UIManager can set its internal categorization_flow_active
    )
...
def _handle_step_3_5_response(buff):
    ...
    if not custom_command:
        append_output_func("⚠️ Command cannot be empty.", style_class='warning')
        _ask_step_3_5_enter_custom_command_for_categorization() # Re-ask
        return
    # Access state through ui_manager_instance
    ui_manager_instance.categorization_flow_state['command_to_add_final'] = custom_command
    append_output_func(f"New command for categorization: '{custom_command}'", style_class='categorize-info')
    ui_manager_instance.categorization_flow_state['step'] = 1
    _ask_step_1_main_action()
...
def _ask_step_4_enter_modified_command(base_command: str):
    ...
    ui_manager_instance.set_flow_input_mode(
        ...
        accept_handler_func=_handle_step_4_modified_command_response,
        is_categorization=True # This flag in UIManager can set its internal categorization_flow_active
    )
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.buffer.text = base_command
        ui_manager_instance.input_field.buffer.cursor_position = len(base_command)
...
def _handle_step_4_modified_command_response(buff):
    ...
    if not modified_command:
        append_output_func("⚠️ Modified command empty.", style_class='warning')
        # If empty, we might want to revert to the original command_to_add_final or re-ask.
        # For now, it will proceed with an empty command if user presses enter.
        # Or, we can keep the original command_to_add_final
        # ui_manager_instance.categorization_flow_state['command_to_add_final'] remains unchanged if empty
    else:
        ui_manager_instance.categorization_flow_state['command_to_add_final'] = modified_command
    ui_manager_instance.categorization_flow_state['step'] = 4.5
...
def _ask_step_4_5_category_for_modified():
    ...
    append_output_func = ui_manager_instance.append_output
    # Access state through ui_manager_instance
    cmd_to_categorize = ui_manager_instance.categorization_flow_state['command_to_add_final']
    ...
    ui_manager_instance.set_flow_input_mode(
        ...
        accept_handler_func=_handle_step_4_5_response,
        is_categorization=True # This flag in UIManager can set its internal categorization_flow_active
    )
...
def _handle_step_4_5_response(buff):
    ...
    if chosen_category:
        ...
        append_output_func("Invalid category. Please enter 1, 2, or 3.", style_class='error')
