...
CATEGORY_DESCRIPTIONS = {
    ...
    "semi_interactive": "Output in micro_X after tmux run (may be interactive)", # Updated description
...
def execute_command_in_tmux(command_to_execute: str, original_user_input_display: str, category: str):
    ...
    try:
        ...
        if category == "semi_interactive":
            ...
            # FIX: Removed "-d" (detach) flag to allow potential interaction
            ...
            # Launch the tmux window. This might take focus depending on tmux/terminal setup.
            # micro_X will then poll in the background.
            ...
            while time.time() - start_time < tmux_poll_timeout:
                time.sleep(1) 
                try:
                    # Check if the tmux window still exists. If not, command likely finished or was closed.
                    ...
                    # If tmux list-windows fails, assume the window is gone or tmux itself has an issue.
                    logger.warning(f"Error checking tmux windows (assuming closed or command done): {tmux_err}")
                    window_closed_or_cmd_done = True
                    break
                
                # Check if log file has content (indicative of command finishing or tee working)
                # This is a secondary check; window closure is primary for "done".
                if os.path.exists(log_path) and os.path.getsize(log_path) > 0:
                     pass # Log content is checked after loop regardless
