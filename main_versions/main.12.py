...
async def handle_input_async(user_input: str): 
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        # Try to get a validated translation from the main AI.
        logger.debug(f"Direct input '{user_input_stripped}' is unknown. Attempting validated AI translation.")
        append_output(f"🔎 Input '{user_input_stripped}' not a known command. Attempting AI interpretation & validation...")
        if get_app().is_running: get_app().invalidate()

        linux_command, ai_raw_candidate = await get_validated_ai_command(user_input_stripped)

        if linux_command:
            append_output(f"🤖 AI Interpreted & Validated to: {linux_command}")
            # If translation happened and is different from original, pass original for user choice
            ...
            # All attempts to get a validated translation failed.
            # Fallback: Treat the original user input as a direct command and prompt for categorization.
