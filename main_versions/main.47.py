...
# from prompt_toolkit.application import get_app # get_app is now called within UIManager
...
from modules.ai_handler import get_validated_ai_command, is_valid_linux_command_according_to_ai, explain_linux_command_with_ai
...
def load_configuration():
    # Loads configurations from default and user JSON files, with a hardcoded fallback.
    global config
...
app_instance = None # Will be set by Application()
current_directory = os.getcwd()
# confirmation_flow_active and confirmation_flow_state are now managed by UIManager (or will be)
# confirmation_flow_active = False # REMOVED - To be managed by UIManager
# confirmation_flow_state = {} # REMOVED - To be managed by UIManager
ui_manager_instance = None # Will be set in main_async_runner
...
def _handle_exit_or_cancel(event):
    ...
    # global confirmation_flow_active, confirmation_flow_state # No longer global here
    global ui_manager_instance # ui_manager_instance is still needed

    is_cat_active = ui_manager_instance.categorization_flow_active if ui_manager_instance else False
    # For confirmation_flow_active, we'll eventually use ui_manager_instance.confirmation_flow_active
    # For now, keeping the old logic structure but it will be simplified once confirmation flow moves
    is_conf_active = ui_manager_instance.confirmation_flow_active if ui_manager_instance and hasattr(ui_manager_instance, 'confirmation_flow_active') else confirmation_flow_active_main_placeholder
    ...
    if is_cat_active:
        ...
        # UIManager's start_categorization_flow future should be resolved
        ...
        restore_normal_input_handler() # This will call UIManager's set_normal_input_mode
        event.app.invalidate()
    elif is_conf_active: # This part will be refactored when confirmation flow moves
        if ui_manager_instance: ui_manager_instance.append_output("\n⚠️ Command confirmation cancelled by user.", style_class='warning')
        logger.info("Confirmation flow cancelled by Ctrl+C/D.")
        # This state will move to UIManager
        # confirmation_flow_active = False # This global will be removed
        if 'future' in confirmation_flow_state_main_placeholder and not confirmation_flow_state_main_placeholder['future'].done():
             confirmation_flow_state_main_placeholder['future'].set_result({'action': 'cancel'})
...
# Placeholder for confirmation_flow_active until it's moved to UIManager
confirmation_flow_active_main_placeholder = False
confirmation_flow_state_main_placeholder = {}
...
def _handle_enter(event):
    ...
    buff.validate_and_handle() # Calls the currently set accept_handler

@kb.add('tab')
def _handle_tab(event):
    # Handles Tab for completion or indentation.
    buff = event.current_buffer
    if buff.complete_state: # If a completion menu is active
        event.app.current_buffer.complete_next()
    else: # Otherwise, insert spaces for indentation
        event.current_buffer.insert_text('    ') 
    ...
    if not is_cat_active and not is_conf_active:
        if doc.cursor_position_row == 0: # If at the first line of current input
            if buff.history_backward(): # Try to go back in history
                # Set cursor to end of history item
                buff.document = Document(text=buff.text, cursor_position=len(buff.text))
                event.app.invalidate()
        else: # If not at the first line, just move cursor up
            buff.cursor_up()
...
def _handle_down_arrow(event):
    ...
    is_conf_active = ui_manager_instance.confirmation_flow_active if ui_manager_instance and hasattr(ui_manager_instance, 'confirmation_flow_active') else confirmation_flow_active_main_placeholder
    if not is_cat_active and not is_conf_active:
        if doc.cursor_position_row == doc.line_count - 1: # If at the last line
            if buff.history_forward(): # Try to go forward in history
                buff.document = Document(text=buff.text, cursor_position=len(buff.text))
                event.app.invalidate()
        else: # If not at the last line, just move cursor down
            buff.cursor_down()

def expand_shell_variables(command_string: str, current_pwd: str) -> str:
    # Expands shell variables like $VAR and ${VAR}, with special handling for $PWD.
    # Placeholder for PWD to avoid expanding it if it's part of another variable name
    pwd_placeholder = f"__MICRO_X_PWD_PLACEHOLDER_{uuid.uuid4().hex}__"
    # Replace $PWD only if it's not followed by other valid variable characters
...
def normal_input_accept_handler(buff):
    # Default accept handler for normal input mode.
    asyncio.create_task(handle_input_async(buff.text))

def restore_normal_input_handler():
    # Restores the input field to normal command input mode via UIManager.
    # global confirmation_flow_active # No longer directly set here
...
async def handle_update_command():
    ...
    try:
        ...
        pull_process = await asyncio.to_thread(subprocess.run, ['git', 'pull', 'origin', current_branch], cwd=SCRIPT_DIR, capture_output=True, text=True) # check=False to handle output
...
async def handle_utils_command_async(full_command_str: str):
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            if not process.stderr: ui_manager_instance.append_output(f"✅ Utility '{script_filename}' completed.", style_class='success') # Only show full success if no stderr
...
def display_general_help():
    ...
    help_text_styled = [ # Using tuples for potential future rich text formatting
    ...
    help_output_string = "".join([text for _, text in help_text_styled]) # Join only the text parts
    ui_manager_instance.append_output(help_output_string, style_class='help-base') # Apply a base style for the whole block
...
async def handle_ollama_command_async(user_input_parts: list):
    # Handles /ollama subcommands like start, stop, status.
    ...
    if subcommand == "start":
        ...
        if success:
            ...
            ollama_service_ready = await ensure_ollama_service(config, append_output_func) # Re-check readiness
        ...
        ollama_service_ready = False # Assume not ready after stop attempt
        ...
        if success:
            ...
            ollama_service_ready = await ensure_ollama_service(config, append_output_func) # Re-check
...
async def handle_input_async(user_input: str):
    # Main handler for user input, routing to appropriate functions.
    global ui_manager_instance, current_directory, ollama_service_ready # confirmation_flow_active removed
    if not ui_manager_instance: logger.error("handle_input_async: UIManager not initialized."); return
    append_output_func = ui_manager_instance.append_output

    # Check if UI manager is in a flow state
    is_cat_active = ui_manager_instance.categorization_flow_active
    is_conf_active = ui_manager_instance.confirmation_flow_active if hasattr(ui_manager_instance, 'confirmation_flow_active') else confirmation_flow_active_main_placeholder # Will be ui_manager_instance.confirmation_flow_active

    if is_cat_active or is_conf_active: # If UIManager is handling a flow, don't process normal input
    ...
    current_app_inst = ui_manager_instance.get_app_instance() # For invalidating UI
    ...
    if user_input_stripped.startswith("/ai "):
        ...
        app_getter = ui_manager_instance.get_app_instance # Pass the getter function
    ...
    if user_input_stripped.startswith("/command"):
        command_action = handle_command_subsystem_input(user_input_stripped) # This function is in category_manager
    ...
    # Direct command or potential natural language query
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL: # Known command
    ...
    else: # Unknown command, try AI validation and then possibly translation
        ...
        # Heuristic check for phrase-like input
        ...
        if user_input_stripped.startswith('$'):
            if len(user_input_stripped) == 1: is_problematic_leading_dollar = True # Just '$'
            elif len(user_input_stripped) > 1 and user_input_stripped[1].isalnum() and user_input_stripped[1] != '{': is_problematic_leading_dollar = True # $VAR without {}
        ...
        elif not has_space: user_input_looks_like_phrase = False # Single word is likely a command
        elif is_command_syntax_present: user_input_looks_like_phrase = False # Contains typical command syntax
        else: user_input_looks_like_phrase = True # Multiple words without clear command syntax
        ...
        if is_cmd_ai_says is True and not user_input_looks_like_phrase:
            ...
            await process_command(user_input_stripped, user_input_stripped, None, None, is_ai_generated=False)
        else: # AI says not a command, or it looks like a phrase despite AI saying it's a command
...
async def process_command(command_str_original: str, original_user_input_for_display: str,
    ...
    # Processes a command string: confirmation (if AI), categorization, expansion, execution.
    # global confirmation_flow_active, confirmation_flow_state # REMOVED - To be managed by UIManager
    global ui_manager_instance, current_directory, confirmation_flow_state_main_placeholder, confirmation_flow_active_main_placeholder # Temporary placeholders
    ...
    if is_ai_generated and not forced_category:
        ...
        # This call will eventually be to ui_manager_instance.prompt_for_command_confirmation
        confirmation_result = await prompt_for_command_confirmation(command_str_original, original_user_input_for_display)
        
        action = confirmation_result.get('action'); confirmed_command = confirmation_result.get('command', command_str_original); chosen_category_from_confirmation = confirmation_result.get('category')
        ...
        if action == 'edit_mode_engaged':
            ...
            # UIManager's set_edit_mode would have been called from within prompt_for_command_confirmation
            return 
        if action == 'execute_and_categorize' and chosen_category_from_confirmation:
        ...
        elif action == 'execute': append_output_func(f"✅ User confirmed execution of: {confirmed_command}", style_class='success'); command_str_original = confirmed_command
        elif action == 'cancel': append_output_func(f"❌ Execution of '{command_str_original}' cancelled.", style_class='info'); logger.info(f"User cancelled execution of AI command: {command_str_original}"); return
        else: # Includes None or unexpected actions
            if action is not None : # Only log error if action was not None (i.e. not a graceful cancel from within flow)
            ...
            return # Abort if flow didn't result in explicit execution

    # Handle 'cd' command directly if not forced category (AI generated 'cd' would have gone through confirmation)
    ...
    if not category: # If category is not forced (e.g. from /command run or AI confirmation with category)
        ...
        if category == UNKNOWN_CATEGORY_SENTINEL:
            ...
            if categorization_result.get('action') == 'cancel_execution': append_output_func(f"Execution of '{command_for_classification}' cancelled.", style_class='info'); logger.info(f"Execution of '{command_for_classification}' cancelled."); return
                ...
                if command_to_be_added_if_new != command_str_original: logger.info(f"Using '{command_to_be_added_if_new}' for execution."); command_str_original = command_to_be_added_if_new # Update if modified during categorization
            else: # Default execution if flow doesn't lead to categorization (e.g. 'execute_as_default')
    ...
    if command_str_original != command_to_execute_expanded:
        ...
        # Only show expansion if it's different from what was classified or what's being added
    ...
    if not command_to_execute_sanitized: append_output_func(f"Command '{command_to_execute_expanded}' blocked.", style_class='security-warning'); logger.warning(f"Command '{command_to_execute_expanded}' blocked."); return
    ...
    if forced_category:
        ...
        else: exec_message_prefix = "Forced execution" # e.g. from /command run
...
# --- Start of Command Confirmation Flow (To be moved to UIManager in Phase 2) ---
async def prompt_for_command_confirmation(command_to_confirm: str, display_source: str) -> dict:
    # This entire function and its helpers will move to UIManager.
    # For now, it uses global placeholders for state.
    global confirmation_flow_active_main_placeholder, confirmation_flow_state_main_placeholder, ui_manager_instance, config
    if not ui_manager_instance: logger.error("prompt_for_command_confirmation: UIManager not initialized."); return {'action': 'cancel'}

    confirmation_flow_active_main_placeholder = True # This will become self.confirmation_flow_active
    confirmation_flow_state_main_placeholder = { # This will become self.confirmation_flow_state
    ...
    _ask_confirmation_main_choice() # This will become self._ask_confirmation_main_choice()

    action_taken = None
    try:
        result = await confirmation_flow_state_main_placeholder['future']
        action_taken = result.get('action')
        return result
    finally:
        confirmation_flow_active_main_placeholder = False # This will become self.confirmation_flow_active = False
        # The UIManager will handle its own state reset for confirmation_flow_active
        # if ui_manager_instance:
        #     ui_manager_instance.confirmation_flow_active = False 

        if action_taken == 'edit_mode_engaged':
            logger.info("Modify action chosen: Setting up edit mode via UIManager.")
            if ui_manager_instance:
                command_for_edit = confirmation_flow_state_main_placeholder.get('command_to_confirm', '')
                ui_manager_instance.set_edit_mode(normal_input_accept_handler, command_for_edit)
        else:
            restore_normal_input_handler() # Calls UIManager's set_normal_input_mode


def _ask_confirmation_main_choice():
    # Part of the confirmation flow - will move to UIManager
    global confirmation_flow_state_main_placeholder, ui_manager_instance
    if not ui_manager_instance: return

    cmd = confirmation_flow_state_main_placeholder['command_to_confirm']; source = confirmation_flow_state_main_placeholder['display_source']
    ui_manager_instance.append_output(f"\n🤖 AI proposed command (from: {source}):", style_class='ai-query')
    ui_manager_instance.append_output(f"    👉 {cmd}", style_class='executing')
    ui_manager_instance.append_output("Action: [Y]es (Exec, prompt if new) | [Ys] Simple & Run | [Ym] Semi-Interactive & Run | [Yi] TUI & Run | [E]xplain | [M]odify | [C]ancel?", style_class='categorize-prompt')

    ui_manager_instance.set_flow_input_mode( # This is already a UIManager method
        prompt_text="[Confirm AI Cmd] Choice (Y/Ys/Ym/Yi/E/M/C): ",
        accept_handler_func=_handle_confirmation_main_choice_response, # This handler will also move
        is_confirmation=True # UIManager will use this flag for its internal state
    )
...
def _handle_confirmation_main_choice_response(buff):
    ...
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    response = buff.text.strip().lower(); valid_choice_made = False
    
    # Accessing confirmation_flow_state_main_placeholder - will be self.confirmation_flow_state
    if response in ['y', 'yes']: confirmation_flow_state_main_placeholder['future'].set_result({'action': 'execute', 'command': confirmation_flow_state_main_placeholder['command_to_confirm']}); valid_choice_made = True
    elif response == 'ys': confirmation_flow_state_main_placeholder['future'].set_result({'action': 'execute_and_categorize', 'command': confirmation_flow_state_main_placeholder['command_to_confirm'], 'category': 'simple'}); valid_choice_made = True
    elif response == 'ym': confirmation_flow_state_main_placeholder['future'].set_result({'action': 'execute_and_categorize', 'command': confirmation_flow_state_main_placeholder['command_to_confirm'], 'category': 'semi_interactive'}); valid_choice_made = True
    elif response == 'yi': confirmation_flow_state_main_placeholder['future'].set_result({'action': 'execute_and_categorize', 'command': confirmation_flow_state_main_placeholder['command_to_confirm'], 'category': 'interactive_tui'}); valid_choice_made = True
    elif response in ['e', 'explain']: confirmation_flow_state_main_placeholder['step'] = 'explain'; asyncio.create_task(_handle_explain_command_async()); valid_choice_made = True # _handle_explain_command_async will move
    elif response in ['m', 'modify']:
        confirmation_flow_state_main_placeholder['future'].set_result({'action': 'edit_mode_engaged'})
        valid_choice_made = True # UIManager's set_edit_mode will be called from prompt_for_command_confirmation
    elif response in ['c', 'cancel', 'n', 'no']: confirmation_flow_state_main_placeholder['future'].set_result({'action': 'cancel'}); valid_choice_made = True
    else:
        append_output_func("Invalid choice. Please enter Y, Ys, Ym, Yi, E, M, or C.", style_class='error')
        _ask_confirmation_main_choice() # Re-ask, this will be self._ask_confirmation_main_choice()
        return
    # If valid choice made, the future is set, and prompt_for_command_confirmation will handle restoring input mode.

async def _handle_explain_command_async():
    # Part of the confirmation flow - will move to UIManager
    global confirmation_flow_state_main_placeholder, ui_manager_instance, config
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    command_to_explain = confirmation_flow_state_main_placeholder['command_to_confirm']
    append_output_func(f"\n🧠 Asking AI to explain: {command_to_explain}", style_class='ai-thinking')
    current_app_inst = ui_manager_instance.get_app_instance()
    if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
    
    explanation = await explain_linux_command_with_ai(command_to_explain, config, append_output_func) # explain_linux_command_with_ai is in ai_handler.py
    if explanation: append_output_func("\n💡 AI Explanation:", style_class='info-header'); append_output_func(explanation, style_class='info')
    else: append_output_func("⚠️ AI could not provide an explanation.", style_class='warning')
    
    _ask_confirmation_after_explain() # This will be self._ask_confirmation_after_explain()
...
def _ask_confirmation_after_explain():
    ...
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    cmd = confirmation_flow_state_main_placeholder['command_to_confirm']
    append_output_func(f"\nCommand to consider: {cmd}", style_class='executing')
    append_output_func("Action: [Y]es (Exec, prompt if new) | [Ys] Simple & Run | [Ym] Semi-Interactive & Run | [Yi] TUI & Run | [M]odify | [C]ancel?", style_class='categorize-prompt')

    ui_manager_instance.set_flow_input_mode(
        prompt_text="[Confirm AI Cmd] Choice (Y/Ys/Ym/Yi/M/C): ",
        accept_handler_func=_handle_confirmation_after_explain_response, # This handler will move
        is_confirmation=True
    )

def _handle_confirmation_after_explain_response(buff):
    # Part of the confirmation flow - will move to UIManager
    global confirmation_flow_state_main_placeholder, ui_manager_instance
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    ...
    else: append_output_func("Invalid choice. Please enter Y, Ys, Ym, Yi, M, or C.", style_class='error'); _ask_confirmation_after_explain(); return
# --- End of Command Confirmation Flow ---
...
async def prompt_for_categorization(command_initially_proposed: str,
    ...
    # This function now delegates entirely to UIManager for the categorization flow.
    global ui_manager_instance, config, current_directory # confirmation_flow_active removed
    ...
    # References to main.py functions for UIManager callbacks are REMOVED
    # as these functions are now part of UIManager.
    # if ui_manager_instance:
    #     ui_manager_instance._main_ask_step_3_5_ref = _ask_step_3_5_enter_custom_command_for_categorization # REMOVED
    #     ui_manager_instance._main_ask_step_4_ref = _ask_step_4_enter_modified_command # REMOVED
    #     ui_manager_instance._main_ask_step_4_5_ref = _ask_step_4_5_category_for_modified # REMOVED
    ...
    # Check if confirmation flow is active (it shouldn't be if categorization just finished,
    # unless categorization was somehow triggered *during* a confirmation, which is unlikely).
    # This logic might need review based on how flows can be nested or interrupt each other.
    is_conf_active = ui_manager_instance.confirmation_flow_active if ui_manager_instance and hasattr(ui_manager_instance, 'confirmation_flow_active') else confirmation_flow_active_main_placeholder
    if not is_conf_active: # Only restore if not in another primary flow
...
# --- Helper functions for categorization flow ---
# These functions are MOVED to UIManager as of this refactoring step.
# _ask_step_0_5_confirm_command_base and _handle_step_0_5_response are ALREADY in UIManager.
# _ask_step_1_main_action and _handle_step_1_main_action_response are ALREADY in UIManager.

# def _ask_step_3_5_enter_custom_command_for_categorization(): # MOVED to UIManager
# def _handle_step_3_5_response(buff): # MOVED to UIManager
# def _ask_step_4_enter_modified_command(base_command: str): # MOVED to UIManager
# def _handle_step_4_modified_command_response(buff): # MOVED to UIManager
# def _ask_step_4_5_category_for_modified(): # MOVED to UIManager
# def _handle_step_4_5_response(buff): # MOVED to UIManager


def handle_cd_command(full_cd_command: str):
    # Handles the 'cd' command to change directory.
    global ui_manager_instance, current_directory
    ...
    try:
        ...
        expanded_dir_arg = os.path.expanduser(os.path.expandvars(target_dir_str)) # Expand ~ and $VARS
        ...
        if os.path.isdir(new_dir_abs):
            ...
            ui_manager_instance.update_input_prompt(current_directory) # Update prompt via UIManager
...
def sanitize_and_validate(command: str, original_input_for_log: str) -> str | None:
    # Basic command sanitization against a list of dangerous patterns.
    global ui_manager_instance
    if not ui_manager_instance: logger.error("sanitize_and_validate: UIManager not initialized."); return command # Allow if UI not up
    append_output_func = ui_manager_instance.append_output
    # More specific rm pattern: -f or -rf followed by / or a variable that might expand to /
    # This is a basic safeguard; true sandboxing is much more complex.
    dangerous_patterns = [
        r'\brm\s+(-[a-zA-Z0-9]*f[a-zA-Z0-9]*|-f[a-zA-Z0-9]*)\s+/\S*', # rm -f /... or rm -rf /...
        r'\bmkfs\b', r'\bdd\b\s+if=/dev/random', r'\bdd\b\s+if=/dev/zero',
        r'\b(shutdown|reboot|halt|poweroff)\b', # Common system power commands
        r'>\s*/dev/sd[a-z]+', # Redirecting output to a raw disk device
        r':\(\)\{:\|:&};:', # Fork bomb
        r'\b(wget|curl)\s+.*\s*\|\s*(sh|bash|python|perl)\b' # Downloading and piping to shell/interpreter
    ]
...
def execute_command_in_tmux(command_to_execute: str, original_user_input_display: str, category: str):
    # Executes a command in a new tmux window, for semi_interactive or interactive_tui.
    ...
    try:
        ...
        if category == "semi_interactive":
            ...
            wrapped_command = f"bash -c '{escaped_command_str}' |& tee {log_path}; sleep {tmux_sleep_after}" # Capture stdout/stderr to log and screen
            ...
            while time.time() - start_time < tmux_poll_timeout:
                ...
                try: # Check if tmux window still exists
                ...
                except (subprocess.CalledProcessError, FileNotFoundError) as tmux_err: logger.warning(f"Error checking tmux windows: {tmux_err}"); window_closed_or_cmd_done = True; break # Assume closed if error
            ...
            if os.path.exists(log_path):
                try:
                    ...
                    # Use configured thresholds for TUI detection
            ...
            try:
                # For interactive_tui, we don't capture output directly here. User interacts in tmux.
...
def execute_shell_command(command_to_execute: str, original_user_input_display: str):
    # Executes a simple shell command and captures its output.
    global ui_manager_instance, current_directory
    ...
    try:
        ...
        # Using Popen for potentially better handling of streams, though communicate() makes it blocking.
        ...
        if process.returncode != 0:
            ...
            # Only show exit code if there wasn't already stderr (which might be more informative)
...
async def main_async_runner():
    ...
    # global confirmation_flow_active, confirmation_flow_state # REMOVED

    ui_manager_instance = UIManager(config) # Instantiate UIManager
    init_category_manager(SCRIPT_DIR, CONFIG_DIR, ui_manager_instance.append_output) # Pass append_output from UIManager
    ...
    # Ensure welcome message is in the UIManager's output buffer if it's empty or doesn't already have it
    initial_buffer_for_ui = list(ui_manager_instance.output_buffer) # Get current buffer from UIManager
    ...
    elif not any(item[1] == initial_welcome_message for item in initial_buffer_for_ui): # Add if not present at all
    ...
    if ui_manager_instance and ui_manager_instance.input_field:
        ...
        # Callback assignments for UIManager to call main.py functions are REMOVED
        # as these functions are now part of UIManager.
        # ui_manager_instance._main_ask_step_3_5_ref = _ask_step_3_5_enter_custom_command_for_categorization # REMOVED
        # ui_manager_instance._main_ask_step_4_ref = _ask_step_4_enter_modified_command # REMOVED
        # ui_manager_instance._main_ask_step_4_5_ref = _ask_step_4_5_category_for_modified # REMOVED
    ...
    app_instance = Application(
        ...
        key_bindings=kb,
        style=ui_manager_instance.style, # Use style from UIManager
        full_screen=True,
        mouse_support=True
    )
...
def run_shell():
    # Entry point to run the shell application.
    try:
