...
async def handle_input_async(user_input: str): 
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        # Matches a space, then a hyphen, then a word char (e.g., " -f") OR starts with hyphen then word char (e.g., "-f")
        ...
        if is_path_indicator:
            ...
            user_input_looks_like_phrase = False # Inputs with options are less likely to be natural language phrases
        elif has_space:
            user_input_looks_like_phrase = True # Has spaces, but no path or option indicators
        # else: no spaces, no path, no options -> not a phrase (e.g. "pwd")
        ...
        if is_cmd_ai_says is True and not user_input_looks_like_phrase:
            ...
            # In all these cases, default to treating as natural language for translation.
