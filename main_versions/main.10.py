...
TRANSLATION_AI_VALIDATED_ATTEMPTS = 2 # How many times to try getting a validated translation
...
async def is_valid_linux_command_according_to_ai(command_text: str) -> bool | None:
    ...
    for i in range(VALIDATOR_AI_ATTEMPTS):
        ...
        if i < VALIDATOR_AI_ATTEMPTS - 1 and len(responses) <= i+1 and responses[-1] is None: 
             await asyncio.sleep(0.5) # Small delay if an attempt was unclear/error before retrying
...
async def handle_input_async(user_input: str): 
    ...
    if user_input_stripped.startswith("/ai "): 
        ...
        # Use the new wrapper for /ai commands
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        user_input_looks_like_phrase = ' ' in user_input_stripped.strip() and \
                                       not user_input_stripped.strip().startswith(('/', './', '../')) and \
                                       not '--' in user_input_stripped and \
                                       not bool(re.search(r'(?:^|\s)-\w', user_input_stripped))
        ...
        if is_cmd_ai_says is True and not user_input_looks_like_phrase:
            ...
            if linux_command:
                ...
                append_output(f"🤔 AI could not produce a validated translation for '{user_input_stripped}'. Treating original input as a potential direct command.")
                logger.info(f"Validated AI translation failed for '{user_input_stripped}'. Proceeding to categorize original input directly.")
                await process_command(user_input_stripped, 
                                      original_user_input_for_display=user_input_stripped, 
                                      ai_raw_candidate=None, # No relevant AI raw candidate for this path
                                      original_direct_input_if_different=None)
...
async def _interpret_and_clean_ai_output(human_input: str) -> tuple[str | None, str | None]:
    ...
    """
    if COMMAND_PATTERN is None: return None, None # Should have been caught earlier
...
async def get_validated_ai_command(human_query: str) -> tuple[str | None, str | None]:
    ...
    Retries the entire translation-validation cycle up to TRANSLATION_AI_VALIDATED_ATTEMPTS.
    ...
    for i in range(TRANSLATION_AI_VALIDATED_ATTEMPTS):
        append_output(f"🧠 AI translation & validation cycle {i+1}/{TRANSLATION_AI_VALIDATED_ATTEMPTS} for: '{human_query}'")
        if get_app().is_running : get_app().invalidate()
        ...
        if cleaned_command:
            ...
            if is_valid_by_validator is True:
                ...
                return cleaned_command, raw_candidate # Return the successfully validated command
        ...
        # If not returned yet (i.e., validation failed or no command from translation) and more attempts left
        if i < TRANSLATION_AI_VALIDATED_ATTEMPTS - 1:
            ...
            logger.error(f"All {TRANSLATION_AI_VALIDATED_ATTEMPTS} translation & validation cycles failed for '{human_query}'.")
            append_output(f"❌ AI failed to produce a validated command for '{human_query}' after {TRANSLATION_AI_VALIDATED_ATTEMPTS} cycles.")
            return None, last_raw_candidate # Return None for command, but the last raw candidate if we got one
