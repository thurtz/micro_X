...
def _handle_exit_or_cancel(event):
    global confirmation_flow_active, confirmation_flow_state
...
def restore_normal_input_handler():
    global ui_manager_instance, current_directory, confirmation_flow_active
    confirmation_flow_active = False
...
async def handle_input_async(user_input: str):
    global ui_manager_instance, current_directory, ollama_service_ready, confirmation_flow_active
...
async def prompt_for_categorization(command_initially_proposed: str,
    ...
    global ui_manager_instance, config, current_directory
    ...
    # For Phase 2, UIManager's start_categorization_flow will call its own _ask_step_0_5_confirm_command_base
    # We also need to pass references to the main.py functions that UIManager will call back for subsequent steps.
    # This is a temporary bridge until those functions are also moved.
    if ui_manager_instance: # Ensure UIManager can store these references if needed
        ui_manager_instance._main_ask_step_1_main_action_ref = _ask_step_1_main_action
        ui_manager_instance._main_ask_step_3_5_ref = _ask_step_3_5_enter_custom_command_for_categorization
        # Add more refs here as needed for other steps still in main.py

    result = await ui_manager_instance.start_categorization_flow(
        command_initially_proposed,
        ai_raw_candidate,
        original_direct_input
        # No longer pass _ask_step_0_5_confirm_command_base
    )
...
# _ask_step_0_5_confirm_command_base and _handle_step_0_5_response are MOVED to UIManager for Phase 2.
# The remaining functions (_ask_step_1_main_action, etc.) are still here for Phase 2
# and will be called by UIManager's _handle_step_0_5_response (via placeholder/ref).
...
def _handle_step_1_main_action_response(buff):
    ...
    if response in ['1', '2', '3']:
        ...
        if future_to_set and not future_to_set.done(): future_to_set.set_result({'action': 'categorize_and_execute', 'command': cmd_to_add, 'category': chosen_category})
    elif response == 'm':
        ui_manager_instance.categorization_flow_state['step'] = 4
        _ask_step_4_enter_modified_command(base_command=cmd_to_add)
    elif response == 'd':
        if future_to_set and not future_to_set.done(): future_to_set.set_result({'action': 'execute_as_default'})
    elif response == 'c':
        if future_to_set and not future_to_set.done(): future_to_set.set_result({'action': 'cancel_execution'})
    else:
...
async def main_async_runner():
    ...
    if ui_manager_instance and ui_manager_instance.input_field:
        ...
        # Pass references to main.py's step functions to UIManager for Phase 2
        # This is a temporary bridge.
        ui_manager_instance._main_ask_step_1_main_action_ref = _ask_step_1_main_action
        ui_manager_instance._main_ask_step_3_5_ref = _ask_step_3_5_enter_custom_command_for_categorization
        ui_manager_instance._main_ask_step_4_ref = _ask_step_4_enter_modified_command
        ui_manager_instance._main_ask_step_4_5_ref = _ask_step_4_5_category_for_modified
