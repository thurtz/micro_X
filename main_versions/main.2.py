...
VALIDATOR_AI_ATTEMPTS = 3 # Number of times to ask the validator AI
...
async def is_valid_linux_command_according_to_ai(command_text: str) -> bool | None:
    """
    Asks AI if the given text is a valid Linux command (best 2 out of 3 attempts).
    Returns True if AI says "yes" (majority), False if "no" (majority), None otherwise.
    """
    ...
    for i in range(VALIDATOR_AI_ATTEMPTS):
        ...
        try:
            ...
            responses.append(None) # Count error as an unclear response
        
        # Small delay between attempts if not the last one
        if i < VALIDATOR_AI_ATTEMPTS - 1 and len(responses) <= i+1 and responses[-1] is None: # if last was unclear/error
    ...
    if yes_count >= (VALIDATOR_AI_ATTEMPTS // 2 + 1): # Majority "yes"
        return True
    elif no_count >= (VALIDATOR_AI_ATTEMPTS // 2 + 1): # Majority "no"
        return False
...
async def handle_input_async(user_input: str): 
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        if is_cmd_ai_says is False:
            append_output(f"💬 AI suggests '{user_input_stripped}' is not a direct command. Attempting as natural language query...")
            logger.info(f"Validator AI suggests '{user_input_stripped}' is not a command. Treating as natural language.")
            should_translate = True
        elif is_cmd_ai_says is True and user_input_looks_like_phrase:
            append_output(f"💬 AI validated '{user_input_stripped}' as command, but it looks like a phrase. Attempting as natural language query first...")
            logger.info(f"Validator AI said '{user_input_stripped}' is a command, but heuristic suggests natural language. Overriding to translation.")
            should_translate = True
        elif is_cmd_ai_says is None: 
            append_output(f"⚠️ AI validation for '{user_input_stripped}' was inconclusive. Attempting as natural language query...")
            logger.warning(f"Validator AI response for '{user_input_stripped}' inconclusive. Defaulting to natural language query.")
            should_translate = True
        
        if should_translate:
            if get_app().is_running: get_app().invalidate()
            linux_command, ai_raw_candidate = interpret_human_input(user_input_stripped) 
            if linux_command:
        ...
        else: # AI said "yes" and it doesn't look like a phrase
            append_output(f"✅ AI believes '{user_input_stripped}' is a direct command.")
