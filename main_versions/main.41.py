...
# Removed unused prompt_toolkit imports if UIManager handles them all
from prompt_toolkit.document import Document
from prompt_toolkit.application import get_app # Still used by some functions if ui_manager_instance is None
...
def _handle_exit_or_cancel(event):
    global categorization_flow_active, categorization_flow_state, confirmation_flow_active, confirmation_flow_state
    global ui_manager_instance

    is_cat_active = ui_manager_instance.categorization_flow_active if ui_manager_instance else categorization_flow_active
    is_conf_active = ui_manager_instance.confirmation_flow_active if ui_manager_instance else confirmation_flow_active
    ...
    if is_cat_active:
        ...
        if ui_manager_instance: ui_manager_instance.categorization_flow_active = False
        categorization_flow_active = False
        ...
        if ui_manager_instance: ui_manager_instance.confirmation_flow_active = False
        confirmation_flow_active = False
...
# --- MODIFICATION: restore_normal_input_handler to use UIManager.update_input_prompt ---
def restore_normal_input_handler():
    global ui_manager_instance, current_directory
    global categorization_flow_active, confirmation_flow_active # These globals are still used for now

    # Reset main.py's global flags
    categorization_flow_active = False
    confirmation_flow_active = False
    
    if ui_manager_instance:
        # Reset UIManager's internal flags as well
        ui_manager_instance.categorization_flow_active = False
        ui_manager_instance.confirmation_flow_active = False

        # Call UIManager to update the prompt
        ui_manager_instance.update_input_prompt(current_directory) # MODIFIED CALL

        if ui_manager_instance.input_field:
            # Set the accept handler and multiline property on UIManager's input_field
            ui_manager_instance.input_field.buffer.accept_handler = normal_input_accept_handler
            ui_manager_instance.input_field.multiline = config.get('behavior', {}).get('input_field_height', 3) > 1
            
            # No need to invalidate here, as update_input_prompt in UIManager should handle it
        else:
            logger.warning("restore_normal_input_handler: ui_manager_instance.input_field is None.")
    else:
...
async def handle_utils_command_async(full_command_str: str):
    ...
    if subcommand_or_script_name.lower() in ["list", "help"]:
        try:
            ...
            if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate(); return # Modified to return here
...
async def handle_input_async(user_input: str):
    global ui_manager_instance, current_directory, ollama_service_ready, categorization_flow_active, confirmation_flow_active
    ...
    if is_cat_active or is_conf_active: logger.warning("Input ignored: categorization or confirmation flow active."); return
...
async def process_command(command_str_original: str, original_user_input_for_display: str,
    ...
    global ui_manager_instance, current_directory, confirmation_flow_active, confirmation_flow_state, categorization_flow_active, categorization_flow_state
    ...
    if is_ai_generated and not forced_category:
        ...
        elif action == 'edit_handled_externally': append_output_func("⌨️ Command loaded into input field for editing.", style_class='info'); return
        ...
        else: append_output_func(f"Internal error in confirmation flow ({action}).", style_class='error'); logger.error(f"Internal error in confirmation flow. Action: {action}"); return
    ...
    if not category:
        ...
        if category == UNKNOWN_CATEGORY_SENTINEL:
            ...
            else: category = config['behavior']['default_category_for_unclassified']; append_output_func(f"Executing '{command_for_classification}' as default '{category}'.", style_class='info'); logger.info(f"Command '{command_for_classification}' executed with default category '{category}'.")
    ...
    if command_str_original != command_to_execute_expanded:
        ...
        if command_to_execute_expanded != command_for_classification and command_to_execute_expanded != command_to_be_added_if_new: append_output_func(f"Expanded for execution: {command_to_execute_expanded}", style_class='info')
...
async def prompt_for_command_confirmation(command_to_confirm: str, display_source: str) -> dict:
    global confirmation_flow_active, confirmation_flow_state, ui_manager_instance, config
    if not ui_manager_instance: logger.error("prompt_for_command_confirmation: UIManager not initialized."); return {'action': 'cancel'}
    append_output_func = ui_manager_instance.append_output
    confirmation_flow_active = True; ui_manager_instance.confirmation_flow_active = True
    confirmation_flow_state = {'command_to_confirm': command_to_confirm, 'original_command': command_to_confirm, 'display_source': display_source, 'step': 'ask_main_choice', 'future': asyncio.Future()}
    if ui_manager_instance.input_field: ui_manager_instance.input_field.multiline = False
    _ask_confirmation_main_choice()
    try: return await confirmation_flow_state['future']
    finally:
        confirmation_flow_active = False
        if ui_manager_instance:
            ui_manager_instance.confirmation_flow_active = False
            if ui_manager_instance.input_field: ui_manager_instance.input_field.multiline = config.get('behavior', {}).get('input_field_height', 3) > 1
        restore_normal_input_handler()

def _ask_confirmation_main_choice():
    global confirmation_flow_state, ui_manager_instance
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    cmd = confirmation_flow_state['command_to_confirm']; source = confirmation_flow_state['display_source']
    append_output_func(f"\n🤖 AI proposed command (from: {source}):", style_class='ai-query')
    append_output_func(f"    👉 {cmd}", style_class='executing')
    append_output_func("Action: [Y]es (Exec, prompt if new) | [Ys] Simple & Run | [Ym] Semi-Interactive & Run | [Yi] TUI & Run | [E]xplain | [M]odify | [C]ancel?", style_class='categorize-prompt')
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.prompt = "[Confirm AI Cmd] Choice (Y/Ys/Ym/Yi/E/M/C): "
        ui_manager_instance.input_field.buffer.reset()
        ui_manager_instance.input_field.buffer.accept_handler = _handle_confirmation_main_choice_response
        current_app_inst = ui_manager_instance.get_app_instance()
        if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
def _handle_confirmation_main_choice_response(buff):
    ...
    if response in ['y', 'yes']: confirmation_flow_state['future'].set_result({'action': 'execute', 'command': confirmation_flow_state['command_to_confirm']}); valid_choice_made = True
    elif response == 'ys': confirmation_flow_state['future'].set_result({'action': 'execute_and_categorize', 'command': confirmation_flow_state['command_to_confirm'], 'category': 'simple'}); valid_choice_made = True
    elif response == 'ym': confirmation_flow_state['future'].set_result({'action': 'execute_and_categorize', 'command': confirmation_flow_state['command_to_confirm'], 'category': 'semi_interactive'}); valid_choice_made = True
    elif response == 'yi': confirmation_flow_state['future'].set_result({'action': 'execute_and_categorize', 'command': confirmation_flow_state['command_to_confirm'], 'category': 'interactive_tui'}); valid_choice_made = True
    elif response in ['e', 'explain']: confirmation_flow_state['step'] = 'explain'; asyncio.create_task(_handle_explain_command_async()); valid_choice_made = True
    elif response in ['m', 'modify']:
        if ui_manager_instance.input_field:
            ui_manager_instance.input_field.buffer.text = confirmation_flow_state['command_to_confirm']
            ui_manager_instance.input_field.buffer.cursor_position = len(ui_manager_instance.input_field.buffer.text)
        confirmation_flow_state['future'].set_result({'action': 'edit_handled_externally'}); valid_choice_made = True
    elif response in ['c', 'cancel', 'n', 'no']: confirmation_flow_state['future'].set_result({'action': 'cancel'}); valid_choice_made = True
    else: append_output_func("Invalid choice. Please enter Y, Ys, Ym, Yi, E, M, or C.", style_class='error'); _ask_confirmation_main_choice(); return
    if valid_choice_made and ui_manager_instance.input_field: ui_manager_instance.input_field.buffer.reset()

async def _handle_explain_command_async():
    global confirmation_flow_state, ui_manager_instance, config
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    command_to_explain = confirmation_flow_state['command_to_confirm']
    append_output_func(f"\n🧠 Asking AI to explain: {command_to_explain}", style_class='ai-thinking')
    current_app_inst = ui_manager_instance.get_app_instance()
    if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
    explanation = await explain_linux_command_with_ai(command_to_explain, config, append_output_func) # Pass UIManager's append_output
...
def _ask_confirmation_after_explain():
    ...
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.prompt = "[Confirm AI Cmd] Choice (Y/Ys/Ym/Yi/M/C): "
        ui_manager_instance.input_field.buffer.reset()
        ui_manager_instance.input_field.buffer.accept_handler = _handle_confirmation_after_explain_response
        current_app_inst = ui_manager_instance.get_app_instance()
        if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
async def prompt_for_categorization(command_initially_proposed: str, ai_raw_candidate_for_suggestions: str | None, original_direct_input_if_different: str | None) -> dict:
    global categorization_flow_active, categorization_flow_state, ui_manager_instance, config
    if not ui_manager_instance: logger.error("prompt_for_categorization: UIManager not initialized."); return {'action': 'cancel_execution'}
    append_output_func = ui_manager_instance.append_output
    categorization_flow_active = True; ui_manager_instance.categorization_flow_active = True
    categorization_flow_state = {'command_initially_proposed': command_initially_proposed, 'ai_raw_candidate': ai_raw_candidate_for_suggestions, 'original_direct_input': original_direct_input_if_different, 'command_to_add_final': command_initially_proposed, 'step': 0.5, 'future': asyncio.Future()}
    if ui_manager_instance.input_field: ui_manager_instance.input_field.multiline = False
    _ask_step_0_5_confirm_command_base()
    try: return await categorization_flow_state['future']
    finally:
        categorization_flow_active = False; is_conf_active = ui_manager_instance.confirmation_flow_active
        if ui_manager_instance:
            ui_manager_instance.categorization_flow_active = False
            if ui_manager_instance.input_field: ui_manager_instance.input_field.multiline = config.get('behavior', {}).get('input_field_height', 3) > 1
            if not is_conf_active: restore_normal_input_handler()
        logger.debug("Categorization flow ended.")

def _ask_step_0_5_confirm_command_base():
    global categorization_flow_state, ui_manager_instance
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    proposed = categorization_flow_state['command_initially_proposed']; original = categorization_flow_state['original_direct_input']
    current_app_inst = ui_manager_instance.get_app_instance()
    if original and original.strip() != proposed.strip():
        ...
        if ui_manager_instance.input_field:
            ui_manager_instance.input_field.prompt = "[Categorize] Choice (1-4): "
            ui_manager_instance.input_field.buffer.accept_handler = _handle_step_0_5_response
            if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
def _handle_step_0_5_response(buff):
    ...
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    response = buff.text.strip(); proposed = categorization_flow_state['command_initially_proposed']; original = categorization_flow_state['original_direct_input']
    if response == '1': categorization_flow_state['command_to_add_final'] = proposed; append_output_func(f"Using processed: '{proposed}'", style_class='categorize-info'); categorization_flow_state['step'] = 1; _ask_step_1_main_action()
    elif response == '2' and original: categorization_flow_state['command_to_add_final'] = original; append_output_func(f"Using original: '{original}'", style_class='categorize-info'); categorization_flow_state['step'] = 1; _ask_step_1_main_action()
    ...
    else: append_output_func("Invalid choice (1-4). Please try again.", style_class='error'); _ask_step_0_5_confirm_command_base(); return
    if response in ['1', '2', '3', '4'] and ui_manager_instance.input_field: ui_manager_instance.input_field.buffer.reset()

def _ask_step_1_main_action():
    global categorization_flow_state, ui_manager_instance, config
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    cmd_display = categorization_flow_state['command_to_add_final']; default_cat_name = config['behavior']['default_category_for_unclassified']
    current_app_inst = ui_manager_instance.get_app_instance()
    ...
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.prompt = "[Categorize] Action (1-3/M/D/C): "
        ui_manager_instance.input_field.buffer.accept_handler = _handle_step_1_main_action_response
        if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
def _handle_step_1_main_action_response(buff):
    ...
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    response = buff.text.strip().lower(); cmd_to_add = categorization_flow_state['command_to_add_final']
    ...
    else: append_output_func("Invalid choice. Please enter 1-3, M, D, or C.", style_class='error'); _ask_step_1_main_action(); return
    if response in ['1', '2', '3', 'm', 'd', 'c'] and ui_manager_instance.input_field: ui_manager_instance.input_field.buffer.reset()
...
def _ask_step_3_5_enter_custom_command_for_categorization():
    ...
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.prompt = "[Categorize] New command: "; ui_manager_instance.input_field.buffer.text = ""
        ui_manager_instance.input_field.buffer.accept_handler = _handle_step_3_5_response
        if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
def _handle_step_3_5_response(buff):
    ...
    if not custom_command: append_output_func("⚠️ Command cannot be empty.", style_class='warning'); _ask_step_3_5_enter_custom_command_for_categorization(); return
    categorization_flow_state['command_to_add_final'] = custom_command; append_output_func(f"New command for categorization: '{custom_command}'", style_class='categorize-info'); categorization_flow_state['step'] = 1; _ask_step_1_main_action()
    if ui_manager_instance.input_field: ui_manager_instance.input_field.buffer.reset()
...
def _ask_step_4_enter_modified_command(base_command: str):
    ...
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.prompt = f"[Categorize] Modified Cmd: "; ui_manager_instance.input_field.buffer.text = base_command; ui_manager_instance.input_field.buffer.cursor_position = len(base_command)
        ui_manager_instance.input_field.buffer.accept_handler = _handle_step_4_modified_command_response
        if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
def _handle_step_4_modified_command_response(buff):
    ...
    if not modified_command: append_output_func("⚠️ Modified command empty.", style_class='warning')
...
def _ask_step_4_5_category_for_modified():
    ...
    append_output_func(f"  1: simple             ({CM_CATEGORY_DESCRIPTIONS['simple']})\n  2: semi_interactive    ({CM_CATEGORY_DESCRIPTIONS['semi_interactive']})\n  3: interactive_tui     ({CM_CATEGORY_DESCRIPTIONS['interactive_tui']})", style_class='categorize-prompt')
    if ui_manager_instance.input_field:
        ui_manager_instance.input_field.prompt = "[Categorize] Category (1-3): "; ui_manager_instance.input_field.buffer.reset()
        ui_manager_instance.input_field.buffer.accept_handler = _handle_step_4_5_response
        if current_app_inst and current_app_inst.is_running: current_app_inst.invalidate()
...
def _handle_step_4_5_response(buff):
    ...
    if not ui_manager_instance: return
    append_output_func = ui_manager_instance.append_output
    response = buff.text.strip(); chosen_category = CM_CATEGORY_MAP.get(response)
    ...
    else: append_output_func("Invalid category. Please enter 1, 2, or 3.", style_class='error'); _ask_step_4_5_category_for_modified(); return
    if ui_manager_instance.input_field: ui_manager_instance.input_field.buffer.reset()

# --- MODIFICATION: handle_cd_command to use UIManager.update_input_prompt ---
def handle_cd_command(full_cd_command: str):
    ...
    try:
        ...
        if os.path.isdir(new_dir_abs):
            ...
            # Call UIManager to update the prompt directly
            ui_manager_instance.update_input_prompt(current_directory) # MODIFIED CALL
            
            append_output_func(f"📂 Changed directory to: {current_directory}", style_class='info')
            ...
            logger.warning(f"Failed cd to '{new_dir_abs}'.")
...
def sanitize_and_validate(command: str, original_input_for_log: str) -> str | None:
    global ui_manager_instance
    if not ui_manager_instance: logger.error("sanitize_and_validate: UIManager not initialized."); return command # Or handle error
    append_output_func = ui_manager_instance.append_output
        ...
        if category == "semi_interactive":
            ...
            if os.path.exists(log_path):
                try:
                    ...
                    tui_line_threshold = config.get('behavior', {}).get('tui_detection_line_threshold_pct', 30.0); tui_char_threshold = config.get('behavior', {}).get('tui_detection_char_threshold_pct', 3.0)
...
async def main_async_runner():
    ...
    global categorization_flow_active, confirmation_flow_active, categorization_flow_state, confirmation_flow_state
...
def run_shell():
    try: asyncio.run(main_async_runner())
