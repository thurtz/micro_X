...
def restore_normal_input_handler():
    ...
    global categorization_flow_active, confirmation_flow_active 
    categorization_flow_active = False
    confirmation_flow_active = False
...
async def process_command(command_str_original: str, original_user_input_for_display: str,
    ...
    if is_ai_generated and not forced_category:
        ...
        # --- MODIFICATION: Check for 'edit_mode_engaged' (was 'edit_handled_externally') ---
        if action == 'edit_mode_engaged':
            ...
            return # Exit process_command, user will edit and resubmit.
        # --- END MODIFICATION ---
        ...
        # Removed 'edit_handled_externally' check here as it's handled above
        ...
        else: # Should only be if action is None or unexpected
            if action is not None : # Only log error if action was something unexpected
            ...
            return # Abort if action is None or unexpected
...
async def prompt_for_command_confirmation(command_to_confirm: str, display_source: str) -> dict:
    ...
    try: 
        ...
        # --- MODIFICATION: Conditional restore based on action ---
        if action_taken == 'edit_mode_engaged':
            logger.info("Modify action chosen: Setting up edit mode via UIManager.")
            if ui_manager_instance:
                ui_manager_instance.set_edit_mode(normal_input_accept_handler)
        else:
            # For all other actions (execute, cancel, etc.), restore fully.
            restore_normal_input_handler()
        # --- END MODIFICATION ---
...
def _handle_confirmation_main_choice_response(buff):
    ...
    # --- MODIFICATION: Change action for Modify ---
        ...
        confirmation_flow_state['future'].set_result({'action': 'edit_mode_engaged'}) # MODIFIED ACTION
    ...
    # --- END MODIFICATION ---
        ...
        _ask_confirmation_main_choice() 
        return 
    # No buffer reset here if flow ends, handled by restore_normal_input_handler or set_edit_mode
...
def _handle_confirmation_after_explain_response(buff):
    ...
    # No buffer reset here if flow ends
...
def _ask_step_0_5_confirm_command_base():
    ...
    if original and original.strip() != proposed.strip():
        ...
        categorization_flow_state['command_to_add_final'] = proposed; categorization_flow_state['step'] = 1
