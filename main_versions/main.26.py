...
LOG_DIR = "logs"
CONFIG_DIR = "config"
DEFAULT_CATEGORY_FILENAME = "default_command_categories.json" # Renamed
USER_CATEGORY_FILENAME = "user_command_categories.json"     # New
...
DEFAULT_CATEGORY_PATH = os.path.join(SCRIPT_DIR, CONFIG_DIR, DEFAULT_CATEGORY_FILENAME) # Path for default
USER_CATEGORY_PATH = os.path.join(SCRIPT_DIR, CONFIG_DIR, USER_CATEGORY_FILENAME)       # Path for user
...
config = {}
DEFAULT_CONFIG_FILENAME = "default_config.json" # General config
USER_CONFIG_FILENAME = "user_config.json"       # General user config
...
def load_configuration():
    ...
    if os.path.exists(default_config_path):
        ...
        try:
            ...
            logger.info(f"Created default general configuration file at {default_config_path}.")
        except Exception as e:
            logger.error(f"Could not create {default_config_path}: {e}", exc_info=True)
...
# This will store the merged view of command categories
_CURRENTLY_LOADED_CATEGORIES = {}


# --- Keybindings (Unchanged from previous version) ---
kb = KeyBindings()
# ... (All @kb.add functions remain the same as in micro_x_main_py_config_reverted) ...
@kb.add('c-c')
@kb.add('c-d')
...
# --- Output Area Management (Unchanged) ---
...
# --- Shell Variable Expansion (Unchanged) ---
...
# --- Normal Input Handler (Unchanged) ---
...
# --- AI Command Validation (Unchanged, uses `config`) ---
...
# --- Update Command Helper (Unchanged) ---
...
async def handle_update_command():
    ...
    if not shutil.which("git"):
        ...
        logger.error("Update command failed: git not found."); return
    ...
    try:
        ...
        append_output(f"ℹ️ Currently on branch: '{current_branch}'. Fetching updates..."); logger.info(f"Current git branch: {current_branch}")
        if get_app().is_running: get_app().invalidate()
        ...
        if pull_process.returncode == 0:
            ...
            if "Already up to date." in pull_process.stdout: append_output("✅ micro_X is already up to date.")
            else:
                append_output("✅ Updates downloaded.")
                if original_req_hash != get_file_hash(REQUIREMENTS_FILE_PATH):
                    requirements_changed = True; append_output("⚠️ requirements.txt has changed."); logger.info("requirements.txt changed during update.")
                ...
                if requirements_changed:
                    ...
                    append_output(f"   cd \"{SCRIPT_DIR}\""); append_output(f"   source .venv/bin/activate"); append_output(f"   pip install -r {REQUIREMENTS_FILENAME}")
    ...
    except subprocess.CalledProcessError as e: append_output(f"❌ Update failed: Error executing git command.\n{e.stderr}"); logger.error(f"Update command git error: {e}", exc_info=True)
    except FileNotFoundError: append_output("❌ Update failed: 'git' command not found."); logger.error("Update command failed: git not found (redundant check).")
    except Exception as e: append_output(f"❌ An unexpected error occurred during update: {e}"); logger.error(f"Unexpected error during update: {e}", exc_info=True)
    finally:
        if get_app().is_running: get_app().invalidate()

# --- Command Handling Logic (classify_command will use the new loading mechanism) ---
async def handle_input_async(user_input: str):
    global current_directory, categorization_flow_active
    if categorization_flow_active: logger.warning("handle_input_async called while categorization_flow_active. Ignoring."); return
    user_input_stripped = user_input.strip(); logger.info(f"Received user input: '{user_input_stripped}'")
    ...
    category = classify_command(user_input_stripped) # Uses new _CURRENTLY_LOADED_CATEGORIES
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        if is_cmd_ai_says is True and not user_input_looks_like_phrase:
            ...
            log_msg = ""; ui_msg = ""
            if is_cmd_ai_says is False: log_msg = f"Validator AI suggests '{user_input_stripped}' is not a command."; ui_msg = f"💬 AI suggests '{user_input_stripped}' is not a direct command. Attempting as natural language query..."
            elif is_cmd_ai_says is True and user_input_looks_like_phrase: log_msg = f"Validator AI confirmed '{user_input_stripped}' as command, but heuristic overrides."; ui_msg = f"💬 AI validated '{user_input_stripped}' as command, but it looks like a phrase. Attempting as natural language query..."
            else: log_msg = f"Validator AI response for '{user_input_stripped}' inconclusive."; ui_msg = f"⚠️ AI validation for '{user_input_stripped}' was inconclusive. Attempting as natural language query..."
...
# --- process_command and categorization flow (Unchanged, uses `config` and `classify_command`) ---
async def process_command(command_str_original: str, original_user_input_for_display: str,
    ...
    category = classify_command(command_for_classification) # Uses the new loading logic via global
    ...
    if category == UNKNOWN_CATEGORY_SENTINEL:
        ...
        if categorization_result.get('action') == 'cancel_execution':
            append_output(f"Execution of '{command_for_classification}' cancelled."); logger.info(f"Execution of '{command_for_classification}' cancelled by user."); return
    ...
    if command_str_original != command_to_execute_expanded:
        logger.info(f"Command after variable expansion: '{command_to_execute_expanded}' (original: '{command_str_original}')")
    ...
    if not command_to_execute_sanitized:
        append_output(f"Command '{command_to_execute_expanded}' blocked by sanitization."); logger.warning(f"Command '{command_to_execute_expanded}' blocked by sanitization."); return
...
async def prompt_for_categorization(command_initially_proposed: str,
    ...
    categorization_flow_state = {'command_initially_proposed': command_initially_proposed, 'ai_raw_candidate': ai_raw_candidate_for_suggestions,
                                 'original_direct_input': original_direct_input_if_different, 'command_to_add_final': command_initially_proposed, 'step': 0.5}
...
def _ask_step_0_5_confirm_command_base():
    ...
    if original and original.strip() != proposed.strip():
        append_output(f"\nSystem processed input to: '{proposed}'\nYour original input was: '{original}'\nWhich version to use as a base for categorization?\n  1: Processed ('{proposed}')\n  2: Original ('{original}')\n  3: Modify a new command string\n  4: Cancel execution")
...
def _handle_step_0_5_response(buff):
    ...
    else: append_output("Invalid choice. Please enter 1-4."); _ask_step_0_5_confirm_command_base(); return
...
def _ask_step_1_main_action():
    ...
    append_output(f"\nCommand '{cmd_display}' is not categorized. Choose an action:\n  1: Add to 'simple'\n  2: Add to 'semi_interactive'\n  3: Add to 'interactive_tui'\n  M: Modify this command string ('{cmd_display}')\n  D: Do not categorize (execute as default '{default_cat_name}')\n  C: Cancel execution")
...
def _handle_step_1_main_action_response(buff):
    ...
    else: append_output("Invalid choice. Please enter 1-3, M, D, or C."); _ask_step_1_main_action(); return
...
def _ask_step_3_5_enter_custom_command_for_categorization():
    ...
    if input_field: input_field.prompt = "[Categorize] New command string: "; input_field.buffer.text = ""; input_field.buffer.accept_handler = _handle_step_3_5_response; get_app().invalidate()

def _handle_step_3_5_response(buff):
    global categorization_flow_state; custom_command = buff.text.strip()
    if not custom_command: append_output("⚠️ Command cannot be empty. Try again or Ctrl+C."); _ask_step_3_5_enter_custom_command_for_categorization(); return
    categorization_flow_state['command_to_add_final'] = custom_command; append_output(f"Proceeding to categorize: '{custom_command}'"); categorization_flow_state['step'] = 1; _ask_step_1_main_action()
...
def _ask_step_4_enter_modified_command(base_command: str):
    ...
    if input_field: input_field.prompt = f"[Categorize] Modified Command: "; input_field.buffer.text = base_command; input_field.buffer.cursor_position = len(base_command); input_field.buffer.accept_handler = _handle_step_4_modified_command_response; get_app().invalidate()

def _handle_step_4_modified_command_response(buff):
    global categorization_flow_state; modified_command = buff.text.strip()
    if not modified_command: append_output("⚠️ Modified command cannot be empty. Using previous.")
...
def _handle_step_4_5_response(buff):
    ...
    if chosen_category:
        categorization_flow_state.get('future').set_result({'action': 'categorize_and_execute', 'command': categorization_flow_state['command_to_add_final'], 'category': chosen_category})
        if input_field: input_field.buffer.reset()
    else: append_output("Invalid category choice. Please enter 1, 2, or 3."); _ask_step_4_5_category_for_modified()

# --- Built-in Command Handlers (handle_cd_command Unchanged) ---
...
# --- sanitize_and_validate, execute_command_in_tmux, execute_shell_command (Unchanged from micro_x_main_py_config_reverted) ---
...
def execute_command_in_tmux(command_to_execute: str, original_user_input_display: str, category: str):
    try:
        ...
        tmux_poll_timeout = config['timeouts']['tmux_poll_seconds']; tmux_sleep_after = config['timeouts']['tmux_semi_interactive_sleep_seconds']
        if category == "semi_interactive":
            log_path = f"/tmp/micro_x_output_{unique_id}.log"; replacement_for_single_quote = "'\"'\"'"; escaped_command_str = command_to_execute.replace("'", replacement_for_single_quote)
            ...
            logger.info(f"Executing semi_interactive tmux: {tmux_cmd_list}"); process = subprocess.Popen(tmux_cmd_list)
...
# --- AI Integration (Unchanged, uses `config`) ---
...
async def _interpret_and_clean_tagged_ai_output(human_input: str) -> tuple[str | None, str | None]:
    ...
    retry_delay = config['behavior']['ai_retry_delay_seconds']; ollama_call_retries = 2; last_exception_in_ollama_call = None
...
async def _get_direct_ai_output(human_input: str) -> tuple[str | None, str | None]:
    ...
    retry_delay = config['behavior']['ai_retry_delay_seconds']; ollama_call_retries = 2; raw_response_content = None; last_exception_in_ollama_call = None
    for attempt in range(ollama_call_retries + 1):
        current_attempt_exception = None
        try:
        ...
        try:
            ...
            for cat_name_key in set(CATEGORY_MAP.values()): # Ensure all defined types exist
                if cat_name_key not in loaded_data:
                    ...
                    logger.warning(f"Category '{cat_name_key}' in {file_path} is not a list. Resetting for this load.")
                    categories[cat_name_key] = []
            ...
            logger.error(f"Error decoding JSON from {file_path}: {e}. Returning empty structure.", exc_info=True)
        except Exception as e:
            logger.error(f"Error loading categories from {file_path}: {e}. Returning empty structure.", exc_info=True)
    else:
        logger.info(f"{file_path} not found. Returning empty category structure.")
...
def load_and_merge_command_categories():
    ...
    if not os.path.exists(DEFAULT_CATEGORY_PATH): # If default was missing, create an empty one
        logger.info(f"{DEFAULT_CATEGORY_FILENAME} was not found. Creating an empty one.")
        try:
            with open(DEFAULT_CATEGORY_PATH, "w") as f:
                json.dump({cat_name: [] for cat_name in set(CATEGORY_MAP.values())}, f, indent=2)
        except Exception as e_create:
            logger.error(f"Could not create empty {DEFAULT_CATEGORY_FILENAME}: {e_create}")


    # Load user categories
    user_categories = _load_single_category_file(USER_CATEGORY_PATH)

    # Start with a deep copy of default categories as the base for merging
    merged_categories = {k: list(v) for k, v in default_categories.items()}

    # Merge user categories, with user's choice taking precedence
    for category_name, user_cmds_in_category in user_categories.items():
        if category_name not in merged_categories: # Should not happen if using CATEGORY_MAP
            merged_categories[category_name] = []
        
        for user_cmd in user_cmds_in_category:
            # Remove this command from ALL categories in merged_categories first
            # to ensure it only appears in the user-specified category.
            for cat_list in merged_categories.values():
                if user_cmd in cat_list:
                    cat_list.remove(user_cmd)
            
            # Add to the target category in merged_categories
    ...
    logger.info("Default and user command categories loaded and merged.")
    # For debugging, you might want to log the merged categories:
    # logger.debug(f"Merged categories: {json.dumps(_CURRENTLY_LOADED_CATEGORIES, indent=2)}")


def classify_command(cmd: str) -> str:
    """Checks the loaded & merged categories to find the classification."""
    if not _CURRENTLY_LOADED_CATEGORIES: # Ensure categories are loaded
...
def _save_user_command_categories(user_data: dict):
    """Saves the provided data structure to the user command categories JSON file."""
    try:
        with open(USER_CATEGORY_PATH, "w") as f:
            json.dump(user_data, f, indent=2)
        logger.info(f"User command categories saved to {USER_CATEGORY_PATH}")
    except Exception as e:
        logger.error(f"Failed to save user command categories to {USER_CATEGORY_PATH}: {e}", exc_info=True)
        append_output(f"❌ Error saving user command categories: {e}")


def add_command_to_category(full_cmd_to_add: str, category_input: str):
    """Adds or updates the category for a given command string IN THE USER FILE."""
    if not full_cmd_to_add:
        append_output("⚠️ Cannot add empty command."); return
    ...
    if not target_category_name:
        append_output(f"❌ Invalid category: '{category_input}'. Use 1/simple, 2/semi_interactive, or 3/interactive_tui."); return

    user_categories = _load_single_category_file(USER_CATEGORY_PATH) # Load current user cats

    # Remove command from any category it might currently be in *within the user file*
    # This ensures the command is only in the new target category in the user file.
    cmd_found_in_user_file_old_cat = None
    for cat_name, cmds_list in user_categories.items():
        if full_cmd_to_add in cmds_list:
            if cat_name != target_category_name: # If it's in a different category
                cmds_list.remove(full_cmd_to_add)
                cmd_found_in_user_file_old_cat = cat_name
            else: # Already in the target category in the user file
                append_output(f"ℹ️ Command '{full_cmd_to_add}' is already explicitly set as '{target_category_name}' by user. No changes to user file.")
                load_and_merge_command_categories() # Reload global cache
                return


    # Add to the target category in the user_categories structure
    if target_category_name not in user_categories: # Should be initialized by _load_single_category_file
    ...
    _save_user_command_categories(user_categories) # Save the modified user categories
    load_and_merge_command_categories() # Reload global cache to reflect change

    if cmd_found_in_user_file_old_cat:
        append_output(f"✅ Command '{full_cmd_to_add}' moved from '{cmd_found_in_user_file_old_cat}' to '{target_category_name}' in your settings.")
    else:
        append_output(f"✅ Command '{full_cmd_to_add}' set as '{target_category_name}' in your settings.")


def remove_command_from_category(full_cmd_to_remove: str):
    """Removes a command string from the user's explicit categorizations."""
    if not full_cmd_to_remove:
        append_output("⚠️ Cannot remove empty command."); return
    ...
    for category_name, commands_in_category in user_categories.items():
        if full_cmd_to_remove in commands_in_category:
            ...
            # Don't break, remove from all lists in user file if somehow duplicated
    
    if found_and_removed_from_user:
        _save_user_command_categories(user_categories)
        load_and_merge_command_categories() # Reload global cache
        append_output(f"🗑️ Command '{full_cmd_to_remove}' removed from your explicit settings. It may revert to a default category if one exists.")
    else:
        append_output(f"⚠️ Command '{full_cmd_to_remove}' not found in your explicit user settings.")


def list_categorized_commands():
    """Displays all categorized commands (merged view), grouped by category."""
...
def move_command_category(full_cmd_to_move: str, new_category_input: str):
    ...
    # This is effectively the same as adding it to the new category,
    # as add_command_to_category now handles removing from old user categories.
...
def handle_command_subsystem_input(input_str: str):
    ...
    cmd_help = ("ℹ️ /command usage:\n"
                ...
                "  Note: Commands are saved to your user-specific categories.\n"
...
def run_shell():
    ...
    # Initial load of command categories
    load_and_merge_command_categories() # Populate _CURRENTLY_LOADED_CATEGORIES
    ...
    def on_output_cursor_pos_changed(_=None):
        ...
        if doc.line_count > render_info.window_height and doc.cursor_position_row < (doc.line_count - render_info.window_height + 1):
            ...
            if not auto_scroll: logger.debug("Auto-scroll enabled (cursor near bottom)."); auto_scroll = True
