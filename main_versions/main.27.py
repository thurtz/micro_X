...
# --- AI Command Validation (Unchanged) ---
...
async def handle_input_async(user_input: str):
    ...
    if user_input_stripped.lower() in {"/help", "help"}: # Added "help" without slash
...
# --- process_command and categorization flow (Unchanged) ---
async def process_command(command_str_original: str, original_user_input_for_display: str,
    ...
    if category == UNKNOWN_CATEGORY_SENTINEL:
        ...
        if categorization_result.get('action') == 'cancel_execution': append_output(f"Execution of '{command_for_classification}' cancelled."); logger.info(f"Execution of '{command_for_classification}' cancelled."); return
            ...
            category = chosen_cat_for_json; logger.info(f"Command '{command_to_be_added_if_new}' categorized as '{category}'.")
    ...
    if command_str_original != command_to_execute_expanded:
        ...
        if command_to_execute_expanded != command_for_classification and command_to_execute_expanded != command_to_be_added_if_new: append_output(f"Expanded for execution: {command_to_execute_expanded}")
    command_to_execute_sanitized = sanitize_and_validate(command_to_execute_expanded, original_user_input_for_display)
    if not command_to_execute_sanitized: append_output(f"Command '{command_to_execute_expanded}' blocked by sanitization."); logger.warning(f"Command '{command_to_execute_expanded}' blocked."); return
...
def _handle_step_4_5_response(buff):
    ...
    if input_field: input_field.buffer.reset() # Reset even on re-ask if needed, or only on success
...
# --- sanitize_and_validate, execute_command_in_tmux, execute_shell_command (Unchanged) ---
...
# --- Command Categorization Subsystem (Unchanged) ---
    ...
    if os.path.exists(file_path):
        try:
            ...
            for cat_name_key in set(CATEGORY_MAP.values()):
                ...
                elif not isinstance(loaded_data[cat_name_key], list): logger.warning(f"Category '{cat_name_key}' in {file_path} not list. Resetting."); categories[cat_name_key] = []
                else: categories[cat_name_key] = [str(cmd) for cmd in loaded_data[cat_name_key] if isinstance(cmd, str)]
...
def load_and_merge_command_categories():
    ...
    for category_name, user_cmds_in_category in user_categories.items():
        ...
        for user_cmd in user_cmds_in_category:
            ...
            if user_cmd not in merged_categories[category_name]: merged_categories[category_name].append(user_cmd)
...
def add_command_to_category(full_cmd_to_add: str, category_input: str):
    ...
    for cat_name, cmds_list in user_categories.items():
        if full_cmd_to_add in cmds_list:
            ...
            else: append_output(f"ℹ️ Command '{full_cmd_to_add}' already set as '{target_category_name}' by user."); load_and_merge_command_categories(); return
...
# --- Main Application Setup and Run (Unchanged) ---
def run_shell():
    ...
    def on_output_cursor_pos_changed(_=None):
        ...
        if doc.line_count > render_info.window_height and doc.cursor_position_row < (doc.line_count - render_info.window_height + 1):
            if auto_scroll: logger.debug("Auto-scroll disabled."); auto_scroll = False
        else:
            if not auto_scroll: logger.debug("Auto-scroll enabled."); auto_scroll = True
