...
TRANSLATION_AI_VALIDATED_ATTEMPTS = 2 # How many times interpret_human_input tries to get a validated translation
...
async def handle_input_async(user_input: str): 
    ...
    if user_input_stripped.startswith("/ai "): 
        ...
        linux_command, ai_raw_candidate = await interpret_human_input_with_validation(human_query) # Uses new wrapper
        if linux_command:
        ...
        else: append_output("🤔 AI could not process the request after validation.")
        return
    ...
    if category != UNKNOWN_CATEGORY_SENTINEL:
        ...
        # Input is not a known categorized command.
        # Try to interpret it as a natural language query using the main AI, with validation.
        logger.debug(f"Direct input '{user_input_stripped}' is unknown. Attempting AI translation with validation.")
        append_output(f"🔎 Input '{user_input_stripped}' not a known command. Attempting AI interpretation...")
        if get_app().is_running: get_app().invalidate()

        linux_command, ai_raw_candidate = await interpret_human_input_with_validation(user_input_stripped)

        if linux_command:
            append_output(f"🤖 AI Interpreted to: {linux_command}")
            await process_command(linux_command,
                                  original_user_input_for_display=f"'{user_input_stripped}' -> {linux_command}",
                                  ai_raw_candidate=ai_raw_candidate)
        else:
            # If translation + validation fails, then treat the original input as a direct command
            # and let process_command handle the interactive categorization for it.
            append_output(f"🤔 AI could not confidently interpret '{user_input_stripped}'. Treating as a potential direct command.")
            logger.info(f"AI interpretation failed for '{user_input_stripped}'. Proceeding to categorize original input as direct command.")
            await process_command(user_input_stripped, original_user_input_for_display=user_input_stripped, ai_raw_candidate=None)
...
async def interpret_human_input_with_validation(human_input: str) -> tuple[str | None, str | None]:
    """
    Tries to get a command from the main AI and validates it using the Validator AI.
    Retries translation if validation fails, up to TRANSLATION_AI_VALIDATED_ATTEMPTS.
    Returns (validated_command, raw_ai_candidate_from_first_successful_regex_match)
    """
    logger.info(f"Attempting validated translation for: '{human_input}'")
    
    for i in range(TRANSLATION_AI_VALIDATED_ATTEMPTS):
        append_output(f"🧠 AI translation cycle {i+1}/{TRANSLATION_AI_VALIDATED_ATTEMPTS} for: '{human_input}'")
        if get_app().is_running : get_app().invalidate()

        # Call the original interpret_human_input (which has its own retries for API calls/parsing)
        # This function will now be renamed to _interpret_and_clean_ai_output
        cleaned_command, raw_candidate = await _interpret_and_clean_ai_output(human_input)

        if cleaned_command:
            append_output(f"🤖 AI translated to: '{cleaned_command}'. Validating...")
            if get_app().is_running : get_app().invalidate()
            ...
            if is_valid_by_validator is True:
                ...
                logger.warning(f"Validator AI rejected translated command '{cleaned_command}'. Retrying translation if attempts left.")
                append_output(f"❌ AI Validator rejected: '{cleaned_command}'.")
            else: # None (inconclusive)
                logger.warning(f"Validator AI inconclusive for translated command '{cleaned_command}'. Retrying translation if attempts left.")
                append_output(f"⚠️ AI Validator inconclusive for: '{cleaned_command}'.")
        else: # _interpret_and_clean_ai_output returned None
            logger.warning(f"Main AI translation (cycle {i+1}) failed to produce a command for '{human_input}'.")
            # No command to validate, loop will continue if attempts left

        if i < TRANSLATION_AI_VALIDATED_ATTEMPTS - 1:
            append_output(f"Retrying translation cycle for '{human_input}'...")
            await asyncio.sleep(1) # Small delay before full re-translation
        else:
            logger.error(f"All {TRANSLATION_AI_VALIDATED_ATTEMPTS} translation cycles failed to produce a validated command for '{human_input}'.")
            append_output(f"❌ AI failed to produce a validated command for '{human_input}' after {TRANSLATION_AI_VALIDATED_ATTEMPTS} cycles.")
            return None, raw_candidate # Return the last raw candidate if any, even if not validated

    return None, None # Should be unreachable if loop runs correctly, but as a fallback


async def _interpret_and_clean_ai_output(human_input: str) -> tuple[str | None, str | None]:
    """
    Internal function to call Ollama, parse response, and apply cleaning stages.
    This is what interpret_human_input used to be, but without the validation loop.
    """
    ...
    # This function will try its own internal retries for API/parsing issues for a single translation attempt
    ollama_call_retries = 2 
    retry_delay_seconds = 1 # Shorter delay for internal retries of a single translation
    ...
    for attempt in range(ollama_call_retries + 1):
        ...
        try:
            logger.info(f"To Ollama (model: {OLLAMA_MODEL}, attempt {attempt + 1}/{ollama_call_retries+1}): '{human_input}'")
            # Run ollama.chat in a separate thread to keep UI responsive
            response = await asyncio.to_thread(
                ollama.chat,
                model=OLLAMA_MODEL, 
                messages=[
            ...
            # If parsing/match failed for this attempt
            if attempt < ollama_call_retries:
                logger.info(f"Retrying Ollama call due to parsing/match failure (internal attempt {attempt + 2}/{ollama_call_retries+1}) for '{human_input}'."); 
                await asyncio.sleep(retry_delay_seconds) # Use await for asyncio.sleep
                continue 
            else: 
                logger.error(f"Ollama parsing/match failed for '{human_input}' after {ollama_call_retries+1} internal attempts. Last AI: {ai_response}"); 
                return None, raw_candidate_from_regex # Return after all internal retries
            ...
            return None, raw_candidate_from_regex # Non-retryable for this function
            ...
            append_output(f"❌ AI Processing Error: {e_gen}"); logger.exception(f"Unexpected error in _interpret_and_clean_ai_output for '{human_input}'") 
        ...
        if current_attempt_exception:
            ...
            if attempt < ollama_call_retries and not isinstance(current_attempt_exception, ollama.ResponseError): 
                logger.info(f"Retrying Ollama call after error '{type(current_attempt_exception).__name__}' (internal attempt {attempt + 2}/{ollama_call_retries+1}) for '{human_input}'."); 
                await asyncio.sleep(retry_delay_seconds) # Use await
            else: 
                logger.error(f"All Ollama call attempts failed for '{human_input}'. Last error: {current_attempt_exception}")
