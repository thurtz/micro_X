...
# micro_X V3 Launcher (LangGraph Edition)
# ------------------
...
SESSION_NAME="micro_x_v3"
if command -v git >/dev/null 2>&1 && [ -d ".git" ]; then
    BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | sed 's/\//_/g')
    SESSION_NAME="micro_x_v3_${BRANCH}"
fi

# Check for tmux
if ! command -v tmux >/dev/null 2>&1; then
    echo "ERROR: tmux is not installed. V3 requires tmux."
    exit 1
fi

# Launch
TMUX_CONFIG="$SCRIPT_DIR/config/.tmux_v3.conf"
LAUNCH_CMD="bash -c 'source .venv/bin/activate && python3 -m micro_X_v3'"

echo "🚀 Launching micro_X V3 in tmux session: $SESSION_NAME"
