...
# --- Main Function ---
def generate_snapshot(summary_message=None): # Added summary_message parameter
    ...
    if summary_message: # Add the summary if provided
    ...
    if not tree_success: # This means the script itself failed, not that the tree is just empty
    ...
    # tests_ran_successfully means the run_tests.py script completed.
    # tests_notice will indicate if tests failed (rc=1) or if the script itself had an error.
    if tests_notice: 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a snapshot of the micro_X project, optionally running prerequisite utilities."
    )
    parser.add_argument(
        "-s", "--summary",
        type=str,
        help="A short summary or reason for generating this snapshot. This will be included in the snapshot file.",
        default=None
    )
    args = parser.parse_args()

    generated_file = generate_snapshot(summary_message=args.summary) # Pass the summary
