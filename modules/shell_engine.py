...
class ShellEngine:
    def __init__(self, config, ui_manager, 
                 ...
                 is_developer_mode: bool = False, # New argument
                 git_context_manager_instance=None # New argument
                 ):
        """
        ...
        # SCRIPT_DIR in main.py is the project root.
        # ShellEngine might need project root for certain operations if they were moved here
        # and previously relied on SCRIPT_DIR from main.py.
        # For now, using a robust way to get project root based on typical structures,
        # but it's best if main.py explicitly passes the project root if ShellEngine needs it.
        module_file_path = os.path.abspath(__file__)
        modules_dir_path = os.path.dirname(module_file_path)
        self.PROJECT_ROOT = os.path.dirname(modules_dir_path) # Assumes shell_engine.py is in modules/
        if not os.path.basename(self.PROJECT_ROOT) == os.path.basename(sys.path[0]): # A basic check
             # Fallback or more robust detection might be needed if structure changes
             # For now, this relies on the typical project structure.
             # Or, better, ShellEngine could take project_root as an __init__ argument from main.py
        ...
        # Ensure UTILS_DIR_PATH exists if ShellEngine is responsible for it
        # However, main.py already does os.makedirs for SCRIPT_DIR/UTILS_DIR_NAME which might be sufficient if SCRIPT_DIR is project root.
        # To be safe, ShellEngine can ensure it too if it directly uses it.
        # For now, this is okay as main.py should have created it.
    ...
    def expand_shell_variables(self, command_string: str) -> str:
        # ... (rest of the ShellEngine methods remain the same as in the snapshot) ...
        # No changes needed in other methods for this specific fix.
        # Ensure to copy the full content of shell_engine.py from the previous snapshot,
        # then apply this __init__ modification.
    ...
    async def execute_command_in_tmux(self, command_to_execute: str, original_user_input_display: str, category: str):
        ...
        try:
            ...
            if category == "semi_interactive":
                ...
                start_time = asyncio.get_event_loop().time() # Use loop time
                ...
                if not output_captured_from_log and not window_closed_or_cmd_done:
    ...
    def _get_file_hash(self, filepath): # Moved from main.py, made method of ShellEngine
    ...
    def _display_general_help(self):
        # ... (Method content as previously, no changes for this fix) ...
    ...
    async def _handle_ollama_command_async(self, user_input_parts: list):
        ...
        if subcommand == "start":
            ...
            if success:
                ...
                await self.ollama_manager_module.ensure_ollama_service(self.config, append_output_func) # Re-check status after start
            ...
            if success:
                ...
                await self.ollama_manager_module.ensure_ollama_service(self.config, append_output_func) # Re-check status after restart
    ...
    async def process_command(self, command_str_original: str, original_user_input_for_display: str,
        ...
        if is_ai_generated and not forced_category:
            ...
            confirmation_result = await self.ui_manager.prompt_for_command_confirmation(
                command_str_original, original_user_input_for_display, self.main_restore_normal_input_ref
            )
            ...
            if action == 'execute_and_categorize' and chosen_category_from_confirmation:
                ...
                if self.main_restore_normal_input_ref: self.main_restore_normal_input_ref(); return
            else:
    ...
    async def submit_user_input(self, user_input: str):
        ...
        if user_input_stripped == "cd" or user_input_stripped.startswith("cd "):
            ...
            await self.handle_cd_command(user_input_stripped); return
        ...
        if user_input_stripped.startswith("/command"):
            command_action = self.category_manager_module.handle_command_subsystem_input(user_input_stripped) # Pass self.ui_manager.append_output if module needs it
        ...
        if category != self.category_manager_module.UNKNOWN_CATEGORY_SENTINEL:
            ...
            if not ollama_is_ready:
                ...
                await self.process_command(user_input_stripped, user_input_stripped, None, None, is_ai_generated=False); return
